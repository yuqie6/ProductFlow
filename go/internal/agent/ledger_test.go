...
func TestFocusCanvasMutationReplayAndConflict(t *testing.T) {
	...
	session, err := as.svc.CreateSession(ctx)
	if err != nil {
		t.Fatal(err)
	}
	convID := session.Conversations[0].ConversationID
	key := clockid.New()
	first, err := as.svc.FocusCanvasTool(ctx, convID, key, []string{"n1"}, nil, nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	replay, err := as.svc.FocusCanvasTool(ctx, convID, key, []string{"n1"}, nil, nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	_, err = as.svc.FocusCanvasTool(ctx, convID, key, []string{"n2"}, nil, nil)
	var conflict apperr.Error
	if !errors.As(err, &conflict) || conflict.Status != 409 {
		t.Fatalf("conflict %v", err)
	}

	applied, err := as.svc.ReconcileGraphTool(ctx, convID, focusCanvasTool, key, map[string]any{}, map[string]any{
	...
	missing, err := as.svc.ReconcileGraphTool(ctx, convID, focusCanvasTool, clockid.New(), map[string]any{}, map[string]any{
		"node_ids": []string{"n1"},
	})
	if err != nil {
		t.Fatal(err)
	...
	empty, err := as.svc.FocusCanvasTool(ctx, convID, clockid.New(), nil, nil, nil)
	if empty != nil {
