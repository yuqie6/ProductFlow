...
func mustSeedMerchantQuota(t *testing.T, db *gorm.DB, merchantID string, units int64) {
	...
	if _, err := svc.Adjust(context.Background(), merchantID, "seed-localedit-"+clockid.New(), units, "test fixture", ""); err != nil {
...
func findAnyLocalEditHoldKey(t *testing.T, db *gorm.DB, merchantID string) string {
	t.Helper()
	var row schema.MerchantQuotaHolds
	err := db.Where("merchant_id = ? AND idempotency_key LIKE ?", merchantID, "local-edit:%").
		Order("created_at DESC").
		Take(&row).Error
	if err != nil {
		t.Fatalf("find localedit hold: %v", err)
	}
