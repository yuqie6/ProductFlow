...
当前公网实例是个人项目的 live demo，项目尚未正式商用发布，也没有真实商户用户。主线使用 User/Merchant 隔离数据；部署初始化后，可配置 SMTP 开启邮箱验证码注册并创建各自商家。普通账号对应一个自有商家，不建设商家切换和团队产品；管理员已具备显式目标商家的商品读、编辑和删除接口，完整管理页面仍待交付，主线变更不代表公网实例已升级。研发主线仍可能需要重建数据库和 storage。
...
- Operator 管理实例配置；邮箱验证注册创建普通账号及自己的商家，不建设多商家切换和团队界面。
- 不提供多组织切换、团队界面、真实支付、自动上架、广告投放或视频生成。公开注册、账号自有商家隔离和内部额度已实现；完整管理后台仍在开发。
...
| 路由 | 用途 |
|---|---|
| `/account` | 本人资料、密码与有效会话 |
| `/ops` | 管理员商家与商品管理、任务记录和额度账本 |
| `/password-recovery` | 邮箱验证码恢复密码 |
| `/home` | 功能导航与真实商品素材展示 |
| `/products` | 商品列表 |
