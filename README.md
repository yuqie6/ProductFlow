...
- 单管理员访问密钥登录，基于 Cookie session 访问后台 API；登录门禁默认开启，可在二次解锁后的设置页关闭。
...
- 独立图片会话支持异步生成任务、队列位置、轻量状态刷新、多候选结果和移动端单栏操作布局。
- 生成图画廊：连续生图结果可保存到 `/gallery`，集中浏览来源、提示词、尺寸、模型和下载入口。
...
- 异步任务：Dramatiq + Redis 投递，PostgreSQL 记录状态，启动时恢复未完成文案/海报任务、商品工作流和连续生图任务。
- 运行中轻量轮询：连续生图和商品工作流 active 时只轮询 status 响应，结束后再刷新完整详情，降低多人同时使用时的前端渲染和后端序列化压力。

仍然不在当前范围内：多用户/多租户、团队权限、支付、托管账号体系、自动投放/自动上架、视频生成、Kubernetes/Helm/发布版容器镜像等生产编排包。仓库内 Docker Compose 自托管路径已可用。
...
- 文档：`docs/PRD.md`、`docs/USER_GUIDE.md`、`docs/ARCHITECTURE.md`、`docs/ARCHITECTURE_HEALTH_REVIEW.md`、`docs/ROADMAP.md`。
...
登录门禁 `admin_access_required` 默认开启：普通工作台和私有 API 需要先用 `ADMIN_ACCESS_KEY` 登录。管理员可在二次解锁 `/settings` 后关闭该开关，让普通工作台/API 免登录访问；但 `ADMIN_ACCESS_KEY` 仍必须保留在环境变量中作为重新开启登录后的管理员入口，`SETTINGS_ACCESS_TOKEN` 也始终独立保护配置页读取和写入。
