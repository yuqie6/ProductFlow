...
- `/settings` 的提示词分组可覆盖商品理解、文案生成、工作台生图、工作台改图和连续生图模板。
...
| 运行前端 lint | 暂无 just 任务 | `pnpm --dir web lint` |
| 运行前端单元测试 | 暂无 just 任务 | `pnpm --dir web test:run` |
...
- `GET /api/products`、`POST /api/products`、`GET /api/products/{product_id}`、`DELETE /api/products/{product_id}`、`GET /api/products/{product_id}/history`
- `POST /api/products/{product_id}/reference-images`、`DELETE /api/source-assets/{asset_id}`、`GET /api/source-assets/{asset_id}/download`
...
- `GET /api/image-sessions`、`POST /api/image-sessions`、`GET/PATCH/DELETE /api/image-sessions/{image_session_id}`
- `POST /api/image-sessions/{image_session_id}/reference-images`、`DELETE /api/image-sessions/{image_session_id}/reference-images/{asset_id}`
- `POST /api/image-sessions/{image_session_id}/generate`、`POST /api/image-sessions/{image_session_id}/assets/{asset_id}/attach-to-product`、`GET /api/image-session-assets/{asset_id}/download`
- `GET /api/products/{product_id}/workflow`、`POST /api/products/{product_id}/workflow/nodes`、`POST /api/products/{product_id}/workflow/edges`、`POST /api/products/{product_id}/workflow/run`
- `PATCH /api/workflow-nodes/{node_id}`、`PATCH /api/workflow-nodes/{node_id}/copy`、`POST /api/workflow-nodes/{node_id}/image`、`POST /api/workflow-nodes/{node_id}/image-source`、`DELETE /api/workflow-nodes/{node_id}`
- `DELETE /api/workflow-edges/{edge_id}`
- `GET /api/settings/lock-state`、`POST /api/settings/unlock`、`GET/PATCH /api/settings`
- `/api/jobs/{job_id}`

图片下载接口支持 `variant` 查询参数，例如 `?variant=original`、`?variant=thumbnail` 或 `?variant=preview`；具体可用变体由后端 storage 中实际生成的文件决定。
