...
ProductFlow 是面向单人或小团队商家的开源自托管商品素材工作台。默认入口现已面向越南 Shopee / TikTok Shop seller 的 LaunchKit：从商品事实生成可手动复制/下载的上架文案、图片证明计划、发布 checklist 和 readiness 分数；原商品画布、AI 文案、AI/模板海报、连续生图会话、生成图画廊和可视化工作流仍作为高级模式保留。

当前形态为私有单管理员实例。自托管部署需要 PostgreSQL、Redis、后端 API、Dramatiq worker、Web 前端，以及可用的文本/图片模型供应商。

## 功能概览

### LaunchKit（越南 Shopee / TikTok Shop）

- 默认登录后进入 `/launch-kits`，面向越南 seller 创建商品 LaunchKit。
- `POST /api/launch-kits` 记录商品根、类目 playbook、目标平台和卖家参考信息。
- `POST /api/launch-kits/{id}/generate` 创建 durable generation task；v1 使用确定性本地生成器，不依赖 Shopee/TikTok API，也不自动上架。
- 生成结果包含 buyer angle、Shopee/TikTok 文案块、图片证明计划、readiness score、发布前 checklist 和 Markdown 导出快照。
- 手动导出是一等路径：详情页支持复制标题/描述/hashtag/整块内容，`GET /api/launch-kits/{id}/exports/markdown` 下载 Markdown。
- Seller feedback 记录是否使用、是否编辑、是否愿意复用/付费和复制/下载指标，帮助后续调整 playbook。
- 队列消息只负责投递；PostgreSQL 中的 LaunchKitGenerationTask 是权威状态，API/worker 启动会恢复 queued 和 stale running 任务。
...
- API/worker 启动时恢复未完成文案/海报任务、LaunchKit generation task、商品工作流和连续生图任务。
...
暂不提供多用户/多租户、团队权限、支付、托管账号体系、Shopee/TikTok API 集成、自动投放/自动上架、视频生成、Kubernetes/Helm/发布版容器镜像等生产编排包。LaunchKit v1 明确以手动导出为主，仓库内 Docker Compose 自托管路径已可用。
...
打开 Web 页面后使用 `ADMIN_ACCESS_KEY` 登录。默认进入 **LaunchKit**；顶部导航仍提供 **商品/工作台**（高级画布模式）、**文/图生图**、**画廊**、**帮助** 和 **配置**。

LaunchKit 本地演示可开启同步生成，避免本机必须启动 worker/Redis 消费消息：
...
生产/长期运行建议保持默认异步模式，由 Dramatiq worker 消费 LaunchKit generation task。
...
| 运行 LaunchKit E2E | 无 just 包装 | `npm --prefix web run e2e` |
