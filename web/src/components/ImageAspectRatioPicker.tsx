    ...
    <div className="space-y-3" data-image-aspect-ratio-picker>
      ...
      {allowCustom ? <div className={`rounded-xl border p-3 ${
        !presetValues.has(value) && parseAspectRatio(value)
          ? "border-accent bg-accent-soft/60 dark:border-accent/40 dark:bg-accent/10"
          : "border-border-l1 bg-surface-base dark:border-border-l1 dark:bg-surface-base/50"
      }`}>
        <div className="mb-2 flex items-center justify-between gap-2">
          <div className="text-xs font-semibold text-text-secondary dark:text-text-primary">{t("imageAspect.custom")}</div>
        ...
        <div className="grid grid-cols-[1fr_auto_1fr] items-end gap-2">
          <label className="block min-w-0">
            <span className="mb-1 block text-[10px] font-semibold text-text-muted dark:text-text-muted">
              {t("imageAspect.width")}
            </span>
            <input
              value={width}
        ...
        <div className={`mt-2 text-[11px] leading-5 ${normalized ? "text-text-muted dark:text-text-muted" : "text-state-error dark:text-state-error"}`}>
