...
import { Check, KeyRound, LogOut, ShieldCheck, UserRound, Palette, Store, Sun, Moon, Monitor } from "lucide-react";
...
export function AccountPage() {
  ...
  return <div className="min-h-screen bg-surface-base text-text-primary">
    ...
    <main className="mx-auto w-full max-w-[1200px] px-4 pt-7 pb-40 sm:px-8 lg:pb-20">
      ...
      <div className="grid items-start gap-6 lg:grid-cols-[220px_minmax(0,1fr)] lg:gap-8">
        <aside className="min-w-0 lg:sticky lg:top-6">
          <div className="mb-5 flex items-center gap-3 border-b border-border-l1 pb-5">
            <span aria-hidden="true" className="flex h-12 w-12 shrink-0 items-center justify-center rounded-panel bg-accent-soft text-xl font-semibold text-accent">{[...(session.data?.user?.display_name || "")][0]?.toLocaleUpperCase(locale) || <UserRound size={22} />}</span>
          ...
          <nav aria-label={t("account.title")} className="flex gap-1 overflow-x-auto pb-1 lg:flex-col">
            {[
              ...
              ...(profile.data?.merchant ? [{id:"merchant", title:accountPreferencesMessage(locale,"merchantTitle"), Icon:Store}] : []),
              {id:"password", title:t("account.security"), Icon:KeyRound},
              {id:"sessions", title:t("account.sessions"), Icon:ShieldCheck},
            ].map(({id,title,Icon}) => <a key={id} href={`#${id}`} className="flex min-h-11 shrink-0 items-center gap-2.5 rounded-control px-3 text-sm text-text-secondary outline-none transition-colors hover:bg-surface-subtle hover:text-text-primary focus-visible:ring-2 focus-visible:ring-focus-ring"><Icon size={16} className="shrink-0 text-text-muted" aria-hidden="true" />{title}</a>)}
          </nav>
        </aside>
        <div className="min-w-0 space-y-6">
      <AccountSection id="profile" title={t("account.profile")} icon={<UserRound size={18} />}>
      ...
      <AccountSection id="preferences" title={accountPreferencesMessage(locale, "title")} icon={<Palette size={18} />}>

        <PreferenceControls />
      </AccountSection>
      {profile.data?.merchant ? <AccountSection id="merchant" title={accountPreferencesMessage(locale, "merchantTitle")} icon={<Store size={18} />}>
      ...
      <AccountSection id="password" title={t("account.security")} icon={<KeyRound size={18} />}>

        <PasswordForm onSuccess={finishSession} />
      </AccountSection>
      <AccountSection id="sessions" title={t("account.sessions")} icon={<ShieldCheck size={18} />}>
...
function ProfileForm({ profile, onSaved }: { profile: AccountProfile; onSaved: (profile: AccountProfile) => void }) {
  ...
  return <form className="space-y-5" onSubmit={(event) => { event.preventDefault(); if (save.isPending) return; const invalid = validateDisplayName(name); setError(invalid ? t(invalid) : ""); if (!invalid) save.mutate(getAccountGeneration()); }}>
    <Input className="!h-11 text-sm" label={t("login.email")} type="email" value={profile.user.email} readOnly autoComplete="username" />
    ...
    <div className="flex flex-wrap items-center justify-end gap-3 border-t border-border-l1 pt-5"><Button type="submit" variant="primary" busy={save.isPending} disabled={name.trim() === profile.user.display_name}>{t("account.save")}</Button>{save.isSuccess ? <p role="status" className="text-sm text-state-success">{t("account.saved")}</p> : null}</div>
  </form>;
}
...
function PasswordForm({ onSuccess }: { onSuccess: (generation: number) => void }) {
  ...
  return <form className="space-y-5" onSubmit={(event) => { event.preventDefault(); if (mutation.isPending) return; const invalid = validateNewPassword(password, confirmation); setError(invalid ? t(invalid) : ""); if (!invalid) mutation.mutate(getAccountGeneration()); }}>
    ...
    {error ? <p role="alert" className="text-sm text-state-error">{error}</p> : null}
    <div className="flex flex-wrap items-center justify-between gap-4 border-t border-border-l1 pt-5"><p className="max-w-sm text-xs leading-5 text-text-muted">{t("account.passwordNote")}</p><Button type="submit" variant="primary" busy={mutation.isPending}>{t("account.changePassword")}</Button></div>
  </form>;
}
      ...
      <div className="grid grid-cols-3 gap-2 sm:gap-4">
        {THEME_PREFERENCES.map(value => {
          ...
          return <label key={value} className="group relative min-w-0 cursor-pointer">
            ...
            <span className="block overflow-hidden rounded-panel border-2 border-border-l1 p-2 transition-colors peer-checked:border-accent peer-focus-visible:ring-2 peer-focus-visible:ring-focus-ring peer-focus-visible:ring-offset-2 peer-disabled:cursor-not-allowed peer-disabled:opacity-50">
              <svg viewBox="0 0 180 110" className="mb-2 w-full rounded-control" aria-hidden="true">
                ...
                <rect x="51" y="12" width="55" height="7" rx="2" fill={value === "dark" ? "#94a3b8" : "#64748b"} />
                <rect x="51" y="30" width="118" height="30" rx="4" fill={value === "dark" ? "#1e293b" : "#e2e8f0"} />
...
function MerchantForm({ merchant, onSaved }: { merchant: SessionMerchant; onSaved: (merchant: SessionMerchant) => void }) {
  ...
  return <form className="space-y-5" onSubmit={(event) => {
    ...
    {error ? <p role="alert" className="text-sm text-state-error">{error}</p> : null}
    <div className="flex flex-wrap items-center justify-end gap-3 border-t border-border-l1 pt-5"><Button type="submit" variant="primary" busy={save.isPending} disabled={suspended || name.trim() === merchant.name}>{t("account.save")}</Button>{save.isSuccess ? <p role="status" className="text-sm text-state-success">{t("account.saved")}</p> : null}</div>
  </form>;
}


function AccountSection({id, title, icon, children}: {id: string; title: string; icon: ReactNode; children: ReactNode}) {
  return <section id={id} tabIndex={-1} aria-labelledby={`${id}-heading`} className="scroll-mt-6 overflow-hidden rounded-surface border border-border-l1 bg-surface-raised shadow-elev-1 outline-none">
    <header className="flex items-center gap-2.5 border-b border-border-l1 px-5 py-4 sm:px-6"><span className="text-accent" aria-hidden="true">{icon}</span><h2 id={`${id}-heading`} className="text-sm font-semibold">{title}</h2></header>
