...
export function RecipeLibraryPanel({
  ...
  if (loading) {
    return <PanelState icon={<Loader2 size={18} className="animate-spin" />} text={t("workflowV2.recipe.loading")} />;
  }
  if (error) {
    return <PanelState text={error} action={t("workflowV2.retry")} onAction={onRetry} />;
  }
  ...
  return (
    <div className="space-y-3 p-3">
        ...
        <section className="rounded-lg border border-emerald-200 bg-emerald-50 p-3 dark:border-emerald-400/30 dark:!bg-emerald-500/10" aria-live="polite">
          <div className="flex items-start gap-2">
            ...
            <div className="min-w-0">
              <div className="text-xs font-semibold text-emerald-900 dark:text-emerald-100">{t("workflowV2.recipe.applied")}</div>
              <div className="mt-1 text-[11px] leading-4 text-emerald-700 dark:text-emerald-200">
                {t("workflowV2.recipe.appliedDetail", { version: application.draft.current_version })}
              </div>
      ...
      {recipes.length === 0 ? (
        <PanelState icon={<Boxes size={19} />} text={t("workflowV2.recipe.empty")} />
      ) : recipes.map((recipe) => {
        ...
        return (
          <article key={recipe.id} className="rounded-lg border border-slate-200 bg-white p-3 shadow-sm dark:border-slate-700 dark:!bg-[#11151d]">
            <div className="flex items-start gap-2.5">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-1 line-clamp-2 text-[11px] leading-4 text-slate-500 dark:text-slate-400">
                  {version.description || t("workflowV2.recipe.noDescription")}
                </p>
              </div>
            </div>
            ...
            <div className="mt-3 flex flex-wrap gap-x-3 gap-y-1 border-y border-slate-100 py-2 text-[10px] font-medium text-slate-500 dark:border-slate-800 dark:text-slate-400">
              <span className="inline-flex items-center gap-1"><Boxes size={11} /> {t("workflowV2.recipe.nodeCount", { count: payload.nodes.length })}</span>
              <span className="inline-flex items-center gap-1"><Images size={11} /> {t("workflowV2.recipe.imageTypeCount", { count: payload.image_types.length })}</span>
              <span>{recipe.kind === "workflow_recipe" ? t("workflowV2.recipe.fullKind") : t("workflowV2.recipe.fragmentKind")}</span>
            </div>
