...
const HELP_DOCS = {
          ...
          blocks: [
            {
              type: "steps",
              items: [
                "商品列表上方展示自有商品与当前已采用商品，并按 Agent 目标、图运行、连续生图、局部编辑分别列出当前工作与近期记录。旧的进行中和等待工作不受日期隐藏；历史失败不等于尚未处理，采用也不等于全部图片质检通过。",
                "从商品列表进入新建页，填写商品名称即可点「开始对话」。仅名称时画布只有商品资料节点，对话侧栏会打开。图种、参考图和商品说明齐了，「开始对话」会带上完整画布并打开对话；「只建画布」落下同一张图，不打开对话。只选了图种或只传了参考图时需要补齐，不会悄悄改成仅名称。上传参考图后可点「帮助生成」看图起草商品说明，规格表按返回内容出现。",
                ...
                "运行整张图时，会按 DAG 处理所有具备必需输入的节点：模板种子和需要更新的生图会生成，已写文稿与未变化的图片会记录为已冻结或已复用。没有必连输入的节点不会入队；没有可入队节点时会提示校验问题。没有互相依赖的节点会同时运行；一张失败不会停掉无依赖的同层镜头。运行内容节点不会直接出图。整图运行期间可以继续改检查器：保存只提交一次，其它节点刚被自动采用不会挡这次保存；同一个节点已被别人改过会提示版本冲突。",
      ...
      sections: [
        {
          ...
          blocks: [
            {
              type: "paragraph",
              text: "图片类型按摄影镜头、信息图、证据图分组。手选时每种默认 2 张。点「应用推荐套图」补齐封面 2、卖点 4、规格 1、选款 1、场景 1、细节 1，已选数量不覆盖。同一图种多张共用一条提示词，卖点图用差异指令分开钩子。资质和工厂是证据图，只占待绑定素材，不会生成。",
            },
            {
              type: "callout",
              title: "参考图在对话里提交",
        ...
        },
        {
          ...
          blocks: [{
            type: "paragraph",
            text: "新建商品时选择「从配方创建」，填写新商品名称、选择已保存完整配方并上传 1 至 6 张参考图，可选填写商品说明。预览取消不创建商品或工作流；点击「确认并创建商品」后直接进入新商品画布，不需要 Agent。网络报错后再次确认不会重复创建同一请求的商品。素材占位不携带原商品图片，进入画布后从新商品图库重新选图，并检查可复用文稿是否适合新商品。已有工作流不允许被完整配方覆盖；片段配方继续从工作台合并。",
          }],
        },
      ],
    },
          ...
          blocks: [
            {
              type: "list",
              items: [
                "商品资料：编辑源商品事实，待确认信息单列；确认后保存。绑定其他商品时会修改该源商品。",
                "参考图片：绑定一张商品图片，设置用途与说明；连线决定使用范围。",
                "创作要求：编辑目标、重点信息、必须包含和禁用要求；待补信息单列。",
                "系列风格：编辑风格关键词和配色，每种颜色可设置用途。",
                "画面方案：按目标、构图与场景、重点内容、文字组织文稿，设置文字模式和语种。可选择一个章节补全或改写，审阅采用后只更新该章节。运行该节点不会生图。无文字模式保留已写标题，执行时不带文字。",
                "图片生成：点击结果卡选择图位，点击眼睛按钮预览并下载。当前图片与已采用版本不同时会提示；导出仍使用已采用版本，明确采用新图后才更新。查看本节点历史版本；比例、清晰度和高级设置只显示当前适配器支持的选项。补充要求不能取消共享约束。未覆盖字段继承方案；修改本图只保存修改项，恢复继承后跟随方案新值。文字可在本图独立设置。运行只出图，不重写共享文稿。导出折叠区单独设置交付尺寸、格式和适配方式，生成交付图不调用模型。",
              ],
            },
          ],
        },
        {
          ...
          blocks: [
            ...
            },
            {
              type: "list",
              items: [
                "主区可在「流程编辑」与「图片成果」间切换。本商品曾切换过则记住上次主视图；否则已有当前图时默认打开成果，无产出或空位时默认流程。清除本地偏好后回退条件默认。成果按用途列出每个生图节点的当前图与状态，未分组与证据图可达；可定位回流程、打开历史、选区运行，并继续局部编辑。切换不建图、不自动开对话、不自动跑模型。",
          ...
          blocks: [
            {
              type: "list",
              items: [
                "提示词：为视觉规范、创作要求、提示词节点和创建页商品说明提供模型。",
  ...
  "en-US": [
    ...
    },
    {
      ...
      icon: Bot,
      sections: [
        { id: "image-plan", title: "Image types and quantities", blocks: [{ type: "paragraph", text: "Image types are grouped as photography, infographic, and evidence. Manual checks default to two images per type. Apply recommended set fills cover 2, selling-point 4, specifications 1, SKU 1, scene 1, and detail 1 without overwriting chosen quantities. Multiple images of one type share one prompt; selling-point frames use variation instructions to split hooks. Certification and factory are evidence placeholders and are not generated." }, { type: "callout", title: "Send references in the conversation", text: "Upload one to six photos that show the real product in the composer, and name the image types. The Agent does not invent a logo, certification, packaging, or factory material that the user did not provide. A complete create form lands the full canvas from either Start conversation or Canvas only." }] },
        ...
        { id: "direct-create", title: "Canvas only", blocks: [{ type: "paragraph", text: "On the create page, enter a product brief plus on-image copy and language, then set an aspect ratio for each image type. Canvas only skips chat and writes a runnable workflow immediately. The brief is written into the creative-brief node. After uploading references, Help fill drafts the brief from the photos and opens a spec table from the returned JSON. The create-page text mode and language are stored on each picture plan. They can later be changed per plan or overridden for one image. Aspect ratio is written per type, such as 3:4 for cover and 1:1 for detail. After you upload references and choose image types, the graph includes product facts, identity photos, visual system, creative brief, and one group per photography or infographic shot. Evidence types are unbound placeholders. Identity photos connect to visual, brief, and generating shots. The workbench opens on details; open the Agent panel when you need it, or add another shot from the add panel. The Agent can explain the graph, check configuration, and request a run; it cannot submit a Draft that replaces the live graph. Run visual, brief, and prompt nodes first so the model writes into the inspector; then run image nodes, a shot, or the whole graph." }] },
        { id: "recipe-create", title: "From a recipe", blocks: [{ type: "paragraph", text: "In New Product, choose From a recipe, enter a product name, select a saved complete recipe, and upload 1 to 6 references. Product details are optional. Cancelling the preview creates nothing; confirmation opens the new canvas without an Agent. Retrying after a network error does not duplicate the same creation request. Asset placeholders do not inherit source images: select new product images in the canvas and review reusable documents. Full recipes cannot overwrite an existing graph; fragments still merge from the workbench." }] },
      ],
    },
    {
      slug: "global-agent",
    ...
    },
    {
      ...
      icon: GitBranch,
      sections: [
        { id: "nodes", title: "Node responsibilities", blocks: [{ type: "list", items: ["Product facts separate confirmed facts from pending observations. Pending or conflicting facts are excluded from generation until confirmed.", "A reference image binds one product-library image with a role and note: product identity, environment, style or evidence.", "Creative brief edits the goal, key messages, required elements and prohibitions. Missing facts appear as read-only questions with a product facts link.", "Series style holds shared style and an editable palette with color roles and notes.", "Picture plans organize the goal, composition, scene, content and text, with advanced details collapsed. The plan owns text mode and language. Complete or rewrite a selected section, then review and adopt its candidate without replacing other sections. Running it does not render images.", "Image generation shows the result and stores only per-picture changes, optional text replacement and model settings. Restoring inheritance removes the local change and follows the plan's latest value. Click a result card to select it; use its eye button to preview and download. A warning identifies differences from the adopted version. Export uses that adopted version until you explicitly adopt the new image. Node image history also supports preview and download. Generation controls follow the active adapter; additional requirements cannot cancel shared constraints. Export settings are separate and process an existing image. Running this node only renders from the live prompt document; a prompt artifact is not required. A reference edge is optional. Authored content nodes are not overwritten on a full-graph run. Direct create wires uploads to visual, brief, prompt, and image nodes."] }] },
    ...
    },
    {
      ...
      icon: Settings,
      sections: [
        { id: "providers", title: "Provider purposes", blocks: [{ type: "list", items: ["Prompt supplies the model used by visual system, creative brief, prompt generation nodes, and create-page source-note drafting.", "Workflow Agent clarifies requirements, organizes the library, and creates workflows.", "Image powers workflow and image-chat generation."] }] },
  ...
  "ja-JP": [
    {
      ...
      icon: BookOpen,
      sections: [
        { id: "current-baseline", title: "現在の作業フロー", blocks: [{ type: "steps", items: [
                "商品一覧の上部には、自分の商家の商品と現在採用されている納品の商品数を表示します。現在の作業と最近の記録は、Agent の目標、グラフ実行、連続画像生成、部分編集に分かれます。以前から進行中・待機中の作業も表示されます。過去の失敗は未対応を意味せず、採用は全画像の品質検査合格を意味しません。","商品作成で商品名を入力し「会話を開始」できます。名称のみのキャンバスは商品資料ノードで、会話パネルが開きます。説明・画像タイプ・参考画像が揃うと、「会話を開始」は完全なキャンバスと会話を開き、「キャンバスのみ」は同じグラフで会話を開きません。画像プランが途中のときは揃える必要があり、名称のみへ黙って落とされません。参考画像を上げたあと「生成を手伝う」で説明を下書きし、返ってきた仕様が表になります。", "ワークベンチの会話に実商品の参考画像を 1 から 6 枚送り、必要な画像を伝えます。Agent が intake を書き、不足を質問します。", "図が実行できる状態になったら、会話サイドバーで Goal を明示開始できます。一度の実行終了は Goal 完了ではありません。完了または解除はユーザーが行います。会話を開いただけでは Goal は作られません。", "「会話を開始」は会話パネルを開きます。「キャンバスのみ」は詳細から入ります。名称のみはまず商品資料ノードです。", "図全体の実行はテンプレート種の視覚・要件・プロンプトだけを補い、更新が必要な出図をやり直します。手書きの原稿は上書きしません。つながっていないノードは同時に出図し、1 枚の失敗で同層の他のカットは止まりません。内容ノードの実行では画像を出しません。", "プロンプト、生成仕様、参考画像の紐付けを編集します。アップロードと生成結果は商品ライブラリへ、長期の横断素材は `/media-library` へ保存されます。"] }] },
    ...
    },
    {
      ...
      icon: Bot,
      sections: [
        { id: "image-plan", title: "画像タイプと枚数", blocks: [{ type: "paragraph", text: "撮影・インフォグラフィック・証拠に分かれます。手選びは各タイプ初期 2 枚。「おすすめセットを適用」はカバー 2、売点 4、仕様 1、SKU 1、シーン 1、ディテール 1 を補い、既存の枚数は上書きしません。同一タイプの複数枚はプロンプトを共有し、売点図は差分指示でフックを分けます。認証と工場は証拠の未バインド枠で、生成しません。" }, { type: "callout", title: "参考画像は会話で送ります", text: "実商品が分かる写真を 1 から 6 枚会話に送り、画像タイプを伝えてください。ユーザーが提供していないロゴ、認証、包装、工場素材を Agent が作り足すことはありません。作成フォームが揃うと、「会話を開始」と「キャンバスのみ」のどちらでも完全なキャンバスになります。" }] },
        ...
        { id: "direct-create", title: "キャンバスのみ", blocks: [{ type: "paragraph", text: "商品作成では商品説明、画像内コピー、言語を記入し、画像タイプごとに比率を選んだうえで「キャンバスのみ」を押すと、会話を開かずに実行可能なワークフローをすぐ作れます。商品説明は創作要件へ書きます。参考画像のあと「生成を手伝う」で説明を下書きし、返ってきた JSON から仕様表が出ます。作成ページの文字モードと言語は各画像プランに保存されます。あとからプラン単位、または画像単位で変更できます。比率はタイプごと（カバー 3:4、ディテール 1:1 など）です。参考画像をアップロードして画像タイプを選ぶと、商品情報、参考画像、ビジュアルシステム、創作要件、プロンプト、画像生成ノードが作成され、参考画像はこれらのノードに接続されます。ワークベンチは詳細から開きます。必要なときに会話を開き、作成ページの画像要件を再提出する必要はありません。不足があれば Agent が会話で尋ねます。ノードと接続の変更はキャンバス上で行います。Agent はキャンバスの説明、設定確認、実行リクエストができますが、既存グラフを上書きする Draft は再提出できません。先にビジュアル、創作要件、プロンプトを実行して検査器へ書き、確認後に画像ノードまたは図全体を実行します。" }] },
        { id: "recipe-create", title: "レシピから作成", blocks: [{ type: "paragraph", text: "商品作成で「レシピから作成」を選び、商品名、保存済みの完全なレシピ、参考画像 1〜6 枚を指定します。商品説明は任意です。プレビューをキャンセルしても商品やグラフは作成されません。確認後は Agent を開かず新商品のキャンバスへ進みます。通信エラー後の同じ確認は商品を重複作成しません。素材の枠は元商品の画像を引き継がないため、新商品の画像を選び直し、再利用する文稿も確認してください。完全なレシピで既存グラフは上書きできません。部分レシピはワークベンチから結合できます。" }] },
      ],
    },
    {
      slug: "global-agent",
    ...
    },
    {
      ...
      icon: GitBranch,
      sections: [
        { id: "nodes", title: "ノードの役割", blocks: [{ type: "list", items: ["商品情報は確認済みの事実と確認待ちを分けます。未確認・矛盾する情報は、確認されるまで生成に使いません。", "参考画像はライブラリの 1 画像と用途・メモを保持します。商品本体、環境、スタイル、証拠を区別します。", "創作要件は目標、訴求点、必須要素、禁止事項を編集します。不足情報は商品情報へのリンク付きの確認待ちとして表示します。", "シリーズスタイルは共通スタイルと、用途・説明付きの配色を編集します。", "画像プランは目的、構図、シーン、内容、文字を編集し、文字モードと言語を管理します。詳細設定は折りたたみます。章を選んで補完・改稿し、候補を確認して採用するとその章だけ更新します。この実行では画像を生成しません。", "画像生成は結果、画像別の差分、文字の置換、モデル設定を保持します。継承に戻すとローカル変更を削除し、プランの最新値に従います。結果カードをクリックして選択し、目のボタンでプレビューとダウンロードができます。採用済みの画像と異なる場合は通知します。新しい画像を明示的に採用するまで、書き出しには採用済みの画像を使用します。画像履歴からもプレビューとダウンロードができます。生成設定は現在のアダプタの対応項目だけを表示し、追加要件は共通制約を解除できません。書き出し設定は分離され、既存画像を処理します。このノードの実行は現在のプロンプト文稿を読んで出図するだけで、成果物は不要です。参考辺は任意です。書き済みの内容ノードは全図実行で上書きしません。直接作成ではアップロード画像をビジュアル、創作要件、プロンプト、画像ノードに接続します。"] }] },
    ...
    },
    {
      ...
      icon: Settings,
      sections: [
        { id: "providers", title: "プロバイダ用途", blocks: [{ type: "list", items: ["プロンプト：ビジュアルシステム、創作要件、プロンプト生成ノード、作成ページの商品説明のモデル。", "ワークフロー Agent：要件確認、ライブラリ整理、ワークフロー作成。", "画像：ワークフローと画像チャットの画像生成。"] }] },
  ...
  "vi-VN": [
    {
      ...
      icon: BookOpen,
      sections: [
        { id: "current-baseline", title: "Cách làm việc hiện tại", blocks: [{ type: "steps", items: [
                "Phần tổng quan phía trên danh sách sản phẩm hiển thị sản phẩm của gian hàng và sản phẩm có bản bàn giao hiện được chọn. Công việc hiện tại và bản ghi gần đây được tách theo mục tiêu Agent, lần chạy sơ đồ, phiên tạo ảnh và chỉnh sửa cục bộ. Công việc cũ đang chạy hoặc đang chờ vẫn hiển thị. Lỗi trong quá khứ không có nghĩa là chưa xử lý; chọn bản bàn giao không có nghĩa mọi ảnh đã qua kiểm tra chất lượng.","Mở trang tạo sản phẩm. Chỉ cần tên là bấm «Bắt đầu hội thoại»: canvas có node thông tin sản phẩm và panel hội thoại mở. Khi mô tả, loại ảnh và ảnh tham chiếu đủ, «Bắt đầu hội thoại» mang canvas đầy đủ và mở chat; «Chỉ tạo canvas» mang cùng đồ thị, không mở chat. Kế hoạch ảnh chưa đủ thì phải bổ sung, không bị âm thầm rút thành chỉ còn tên. Sau khi tải ảnh, «Hỗ trợ điền» có thể nháp mô tả từ ảnh; hàng thông số lấy từ JSON trả về.", "Tải từ một đến sáu ảnh tham chiếu của sản phẩm thật trong hộp thoại, và nói cần những ảnh nào. Agent ghi intake và hỏi phần còn thiếu.", "Khi đồ thị đã chạy được, bắt đầu Goal ở thanh hội thoại. Một lần chạy xong chưa phải Goal hoàn thành; bạn bấm hoàn thành hoặc xóa. Mở hội thoại không tự tạo Goal.", "«Bắt đầu hội thoại» mở panel Agent. «Chỉ tạo canvas» vào workbench ở tab chi tiết. Chỉ có tên thì trước hết có node thông tin sản phẩm.", "Chạy cả đồ thị chỉ bổ sung brief, visual và prompt còn là hạt giống, rồi vẽ lại ảnh đã cũ. Bản thảo đã viết không bị ghi đè. Các node không nối với nhau chạy cùng lúc; một ảnh thất bại không dừng các shot cùng lớp. Chạy node nội dung không xuất hình.", "Chỉnh prompt, thông số tạo và liên kết ảnh tham chiếu. Ảnh tải lên và kết quả tạo thuộc thư viện sản phẩm; tài nguyên dài hạn xuyên sản phẩm nằm ở `/media-library`."] }] },
    ...
    },
    {
      ...
      icon: Bot,
      sections: [
        { id: "image-plan", title: "Loại ảnh và số lượng", blocks: [{ type: "paragraph", text: "Chia thành ảnh chụp, infographic và bằng chứng. Chọn tay mỗi loại mặc định 2 ảnh. Áp dụng bộ đề xuất bổ sung bìa 2, điểm bán 4, thông số 1, SKU 1, cảnh 1, chi tiết 1, không ghi đè số lượng đã chọn. Nhiều ảnh cùng loại dùng chung một prompt; ảnh điểm bán dùng chỉ dẫn biến thể để tách móc. Chứng nhận và nhà máy là khung bằng chứng, không sinh ảnh." }, { type: "callout", title: "Gửi ảnh tham chiếu trong hội thoại", text: "Tải một đến sáu ảnh cho thấy sản phẩm thật vào hộp thoại và nói loại ảnh. Agent không tự tạo logo, chứng nhận, bao bì hoặc tư liệu nhà máy mà người dùng chưa cung cấp. Biểu mẫu đủ thì «Bắt đầu hội thoại» và «Chỉ tạo canvas» đều mang canvas đầy đủ." }] },
        ...
        { id: "direct-create", title: "Chỉ tạo canvas", blocks: [{ type: "paragraph", text: "Trên trang tạo, nhập mô tả sản phẩm cùng nội dung trên ảnh và ngôn ngữ, rồi đặt tỷ lệ cho từng loại ảnh. «Chỉ tạo canvas» bỏ qua hội thoại và ghi ngay một quy trình chạy được. Mô tả được ghi vào node brief. Sau khi tải ảnh, «Hỗ trợ điền» nháp mô tả từ ảnh và mở bảng thông số từ JSON trả về. Chế độ chữ và ngôn ngữ trên trang tạo được lưu vào từng phương án ảnh. Có thể đổi theo phương án hoặc ghi đè riêng cho một ảnh. Tỷ lệ ghi theo loại, ví dụ 3:4 cho ảnh bìa và 1:1 cho ảnh chi tiết. Sau khi tải ảnh tham chiếu và chọn loại ảnh, đồ thị gồm thông tin sản phẩm, ảnh tham chiếu, hệ thống hình ảnh, brief sáng tạo, node prompt và node ảnh, với ảnh nối vào các node đó. Workbench mở ở tab chi tiết; cần thì mở hội thoại. Ảnh đã gắn vào sản phẩm, không cần nộp lại yêu cầu ảnh ở trang tạo. Thiếu gì Agent hỏi trong hội thoại. Sửa node và dây trên canvas. Agent có thể giải thích đồ thị, kiểm tra cấu hình và xin chạy; không được nộp Draft khác để ghi đè đồ thị hiện có. Chạy hệ thống hình ảnh, brief và prompt trước để ghi vào inspector; rồi chạy node ảnh hoặc toàn đồ." }] },
        { id: "recipe-create", title: "Tạo từ công thức", blocks: [{ type: "paragraph", text: "Trong Tạo sản phẩm, chọn Tạo từ công thức, nhập tên sản phẩm, chọn công thức đầy đủ đã lưu và tải 1 đến 6 ảnh tham chiếu. Chi tiết sản phẩm không bắt buộc. Hủy xem trước không tạo sản phẩm hay đồ thị; xác nhận mở canvas mới mà không cần Agent. Thử lại cùng yêu cầu sau lỗi mạng không tạo sản phẩm trùng. Ô ảnh không kế thừa ảnh nguồn: chọn ảnh của sản phẩm mới và kiểm tra nội dung tái sử dụng. Công thức đầy đủ không ghi đè đồ thị hiện có; công thức bộ phận vẫn được ghép từ workbench." }] },
      ],
    },
    {
      slug: "global-agent",
    ...
    },
    {
      ...
      icon: GitBranch,
      sections: [
        { id: "nodes", title: "Vai trò của node", blocks: [{ type: "list", items: ["Thông tin sản phẩm tách dữ kiện đã xác nhận và thông tin chờ xác nhận. Thông tin chưa xác nhận hoặc mâu thuẫn không được dùng để tạo ảnh.", "Ảnh tham chiếu giữ một ảnh thư viện cùng vai trò và ghi chú: sản phẩm, bối cảnh, phong cách hoặc bằng chứng.", "Brief sáng tạo chỉnh sửa mục tiêu, thông điệp, yếu tố bắt buộc và điều cấm. Dữ kiện còn thiếu là câu hỏi chỉ đọc, có liên kết đến thông tin sản phẩm.", "Phong cách bộ ảnh gồm phong cách chung và bảng màu có mục đích, ghi chú và ô chọn màu.", "Phương án ảnh tổ chức mục tiêu, bố cục, bối cảnh, nội dung và chữ; quản lý chế độ chữ và ngôn ngữ. Chi tiết nâng cao được thu gọn. Chọn một mục để bổ sung hoặc viết lại, rồi xem và áp dụng bản đề xuất chỉ cho mục đó. Chạy node này không tạo ảnh.", "Node tạo ảnh hiển thị kết quả, lưu thay đổi riêng, chữ thay thế và cài đặt mô hình. Khôi phục kế thừa xóa thay đổi riêng và dùng giá trị mới nhất của phương án. Nhấp thẻ kết quả để chọn, dùng nút hình mắt để xem trước và tải xuống. Khi ảnh khác phiên bản đã chọn, giao diện sẽ thông báo. Xuất vẫn dùng phiên bản đã chọn cho đến khi bạn chọn ảnh mới rõ ràng. Lịch sử ảnh cũng hỗ trợ xem trước và tải xuống. Thiết lập tạo ảnh chỉ hiện tùy chọn được bộ điều hợp hiện tại hỗ trợ; yêu cầu bổ sung không hủy ràng buộc chung. Thiết lập xuất ảnh được tách riêng và xử lý ảnh hiện có. Chạy node này chỉ xuất hình từ văn bản prompt hiện tại, không cần artifact. Cạnh tham chiếu là tùy chọn. Node nội dung đã viết không bị chạy cả đồ thị ghi đè. Tạo trực tiếp nối ảnh tải lên vào hệ thống hình ảnh, brief, prompt và node ảnh."] }] },
    ...
    },
    {
      ...
      icon: Settings,
      sections: [
        { id: "providers", title: "Mục đích nhà cung cấp", blocks: [{ type: "list", items: ["Prompt cung cấp mô hình cho hệ thống hình ảnh, brief sáng tạo, node tạo prompt và nháp mô tả trên trang tạo.", "Agent quy trình làm rõ yêu cầu, sắp xếp thư viện và tạo quy trình.", "Ảnh cung cấp khả năng tạo ảnh cho quy trình và phiên tạo ảnh."] }] },
