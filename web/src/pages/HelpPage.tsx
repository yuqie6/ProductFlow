      ...
      sections: [
        ...
        },
        {
          ...
          blocks: [
            ...
            {
              type: "paragraph",
              text: "节点使用按角色分的输入端口和输出连接点。拖线时合法目标变绿；不兼容端口无法吸附。可拖动已有边的端点换端口。缺必连输入时端口标红、运行按钮禁用。还没有工作流时可用从空白建图写入空图，再添加六类节点。节点较多时可用分组收拢局部流程；双击分组进入后只看组内节点，面包屑返回全图。分组只改善布局，不改变执行顺序。",
            },
            {
              type: "list",
              items: [
                ...
                "悬停画布运行按钮会给将生成、复用、冻结或阻塞的节点着色；点击立即提交。已有运行进行中时新请求进入排队。",
