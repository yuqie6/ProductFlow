  ...
  "ja-JP": [
    ...
    },
    {
      ...
      sections: [
        ...
        { id: "direct-create", title: "キャンバスのみ", blocks: [{ type: "paragraph", text: "商品作成では商品説明、画像内コピー、言語を記入し、画像タイプごとに比率を選んだうえで「キャンバスのみ」を押すと、会話を開かずに実行可能なワークフローをすぐ作れます。商品説明は創作要件へ書きます。撮影ショットは初期で文字なし、売点・仕様などの資料ページは常に文字を入れ、言語は出図設定（既定は簡体字中国語）です。比率はタイプごと（カバー 3:4、ディテール 1:1 など）です。参考画像をアップロードして画像タイプを選ぶと、商品情報、参考画像、ビジュアルシステム、創作要件、プロンプト、画像生成ノードが作成され、参考画像はこれらのノードに接続されます。ワークベンチは詳細から開きます。必要なときに会話を開き、作成ページの画像要件を再提出する必要はありません。不足があれば Agent が会話で尋ねます。ノードと接続の変更はキャンバス上で行います。Agent はキャンバスの説明、設定確認、実行リクエストができますが、既存グラフを上書きする Draft は再提出できません。先にビジュアル、創作要件、プロンプトを実行して検査器へ書き、確認後に画像ノードまたは図全体を実行します。" }] },
      ],
    },
    {
      slug: "global-agent",
  ...
  "vi-VN": [
    ...
    },
    {
      ...
      sections: [
        ...
        { id: "direct-create", title: "Chỉ tạo canvas", blocks: [{ type: "paragraph", text: "Trên trang tạo, nhập mô tả sản phẩm cùng nội dung trên ảnh và ngôn ngữ, rồi đặt tỷ lệ cho từng loại ảnh. «Chỉ tạo canvas» bỏ qua hội thoại và ghi ngay một quy trình chạy được. Mô tả được ghi vào node brief. Ảnh chụp mặc định không chữ; trang điểm bán và thông số luôn có chữ, dùng ngôn ngữ đã chọn (mặc định tiếng Trung giản thể). Tỷ lệ ghi theo loại, ví dụ 3:4 cho ảnh bìa và 1:1 cho ảnh chi tiết. Sau khi tải ảnh tham chiếu và chọn loại ảnh, đồ thị gồm thông tin sản phẩm, ảnh tham chiếu, hệ thống hình ảnh, brief sáng tạo, node prompt và node ảnh, với ảnh nối vào các node đó. Workbench mở ở tab chi tiết; cần thì mở hội thoại. Ảnh đã gắn vào sản phẩm, không cần nộp lại yêu cầu ảnh ở trang tạo. Thiếu gì Agent hỏi trong hội thoại. Sửa node và dây trên canvas. Agent có thể giải thích đồ thị, kiểm tra cấu hình và xin chạy; không được nộp Draft khác để ghi đè đồ thị hiện có. Chạy hệ thống hình ảnh, brief và prompt trước để ghi vào inspector; rồi chạy node ảnh hoặc toàn đồ." }] },
      ],
    },
    {
      slug: "global-agent",
