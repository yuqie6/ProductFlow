          ...
          blocks: [
            {
              ...
              items: [
                ...
                "图片生成：保存比例、分辨率、质量、背景、文字策略、参考保真度和执行状态。文字策略会约束提示词生成和出图。摄影图默认不要文案；资料页始终要文案并带语种。画布上新加的生图节点：摄影不要文案，信息图要文案。运行该节点只出图，读取当前提示词文稿，不要求上游产物。无参考边也可以跑。已写的内容节点不会被整图覆盖。直接创建会把上传图接到视觉规范、创作要求、提示词和生图节点。",
              ],
            },
          ],
        },
  ...
  "ja-JP": [
    ...
    },
    {
      ...
      sections: [
        ...
        { id: "direct-create", title: "キャンバスのみ", blocks: [{ type: "paragraph", text: "商品作成では商品説明、画像内コピー、言語を記入し、画像タイプごとに比率を選んだうえで「キャンバスのみ」を押すと、会話を開かずに実行可能なワークフローをすぐ作れます。商品説明は創作要件へ書きます。参考画像のあと「生成を手伝う」で説明を下書きし、返ってきた JSON から仕様表が出ます。撮影ショットは初期で文字なし、売点・仕様などの資料ページは常に文字を入れ、言語は出図設定（既定は簡体字中国語）です。比率はタイプごと（カバー 3:4、ディテール 1:1 など）です。参考画像をアップロードして画像タイプを選ぶと、商品情報、参考画像、ビジュアルシステム、創作要件、プロンプト、画像生成ノードが作成され、参考画像はこれらのノードに接続されます。ワークベンチは詳細から開きます。必要なときに会話を開き、作成ページの画像要件を再提出する必要はありません。不足があれば Agent が会話で尋ねます。ノードと接続の変更はキャンバス上で行います。Agent はキャンバスの説明、設定確認、実行リクエストができますが、既存グラフを上書きする Draft は再提出できません。先にビジュアル、創作要件、プロンプトを実行して検査器へ書き、確認後に画像ノードまたは図全体を実行します。" }] },
    ...
    },
    {
      ...
      icon: GitBranch,
      sections: [
        { id: "nodes", title: "ノードの役割", blocks: [{ type: "list", items: ["商品情報ノードは確認済みの商品事実を提供します。", "参考画像ノードは商品ライブラリの具体的な 1 画像を保持します。", "創作要件ノードは実行時に商品情報と参考画像から目標と制限を書き、検査器で再編集できます。", "ビジュアルシステムノードは実行時にスタイルと背景を書き、検査器で再編集できます。", "プロンプト生成ノードは実行時にプロンプトを検査器へ書きます。この実行では画像を出しません。", "画像生成ノードは比率、品質、文字方針、背景、参考忠実度、実行状態を保持します。撮影は初期でコピーなし、資料ページは常にコピーと言語です。キャンバスで追加した撮影ノードはコピーなし、インフォグラフィックはコピー必須です。このノードの実行は現在のプロンプト文稿を読んで出図するだけで、成果物は不要です。参考辺は任意です。書き済みの内容ノードは全図実行で上書きしません。直接作成ではアップロード画像をビジュアル、創作要件、プロンプト、画像ノードに接続します。"] }] },
  ...
  "vi-VN": [
    ...
    },
    {
      ...
      sections: [
        ...
        { id: "direct-create", title: "Chỉ tạo canvas", blocks: [{ type: "paragraph", text: "Trên trang tạo, nhập mô tả sản phẩm cùng nội dung trên ảnh và ngôn ngữ, rồi đặt tỷ lệ cho từng loại ảnh. «Chỉ tạo canvas» bỏ qua hội thoại và ghi ngay một quy trình chạy được. Mô tả được ghi vào node brief. Sau khi tải ảnh, «Hỗ trợ điền» nháp mô tả từ ảnh và mở bảng thông số từ JSON trả về. Ảnh chụp mặc định không chữ; trang điểm bán và thông số luôn có chữ, dùng ngôn ngữ đã chọn (mặc định tiếng Trung giản thể). Tỷ lệ ghi theo loại, ví dụ 3:4 cho ảnh bìa và 1:1 cho ảnh chi tiết. Sau khi tải ảnh tham chiếu và chọn loại ảnh, đồ thị gồm thông tin sản phẩm, ảnh tham chiếu, hệ thống hình ảnh, brief sáng tạo, node prompt và node ảnh, với ảnh nối vào các node đó. Workbench mở ở tab chi tiết; cần thì mở hội thoại. Ảnh đã gắn vào sản phẩm, không cần nộp lại yêu cầu ảnh ở trang tạo. Thiếu gì Agent hỏi trong hội thoại. Sửa node và dây trên canvas. Agent có thể giải thích đồ thị, kiểm tra cấu hình và xin chạy; không được nộp Draft khác để ghi đè đồ thị hiện có. Chạy hệ thống hình ảnh, brief và prompt trước để ghi vào inspector; rồi chạy node ảnh hoặc toàn đồ." }] },
    ...
    },
    {
      ...
      icon: GitBranch,
      sections: [
        { id: "nodes", title: "Vai trò của node", blocks: [{ type: "list", items: ["Node thông tin sản phẩm cung cấp dữ kiện đã xác nhận.", "Mỗi node ảnh tham chiếu giữ đúng một ảnh trong thư viện sản phẩm.", "Node brief sáng tạo khi chạy ghi mục tiêu và giới hạn vào inspector.", "Node hệ thống hình ảnh khi chạy ghi phong cách và nền vào inspector.", "Node tạo prompt khi chạy ghi prompt vào inspector và không xuất hình.", "Node tạo ảnh lưu tỷ lệ, chất lượng, chính sách chữ, nền, độ trung thành tham chiếu và trạng thái chạy. Ảnh chụp mặc định không chữ; trang infographic luôn có chữ và ngôn ngữ. Node ảnh chụp thêm trên canvas mặc định không chữ; node infographic mặc định bắt buộc có chữ. Chạy node này chỉ xuất hình từ văn bản prompt hiện tại, không cần artifact. Cạnh tham chiếu là tùy chọn. Node nội dung đã viết không bị chạy cả đồ thị ghi đè. Tạo trực tiếp nối ảnh tải lên vào hệ thống hình ảnh, brief, prompt và node ảnh."] }] },
