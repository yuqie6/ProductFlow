          ...
          blocks: [
            {
              ...
              items: [
                ...
                "运行整张图时，按依赖运行视觉规范、创作要求和提示词，再运行生图。没有互相连线的节点会同时出图；一张失败不会停掉同层其他镜头。运行内容节点不会直接出图。",
  ...
  "vi-VN": [
    {
      ...
      icon: BookOpen,
      sections: [
        { id: "current-baseline", title: "Cách làm việc hiện tại", blocks: [{ type: "steps", items: ["Mở trang tạo sản phẩm. Chỉ cần tên là bấm «Bắt đầu hội thoại»: canvas có node thông tin sản phẩm và panel hội thoại mở. Khi mô tả, loại ảnh và ảnh tham chiếu đủ, «Bắt đầu hội thoại» mang canvas đầy đủ và mở chat; «Chỉ tạo canvas» mang cùng đồ thị, không mở chat. Kế hoạch ảnh chưa đủ thì phải bổ sung, không bị âm thầm rút thành chỉ còn tên.", "Tải từ một đến sáu ảnh tham chiếu của sản phẩm thật trong hộp thoại, và nói cần những ảnh nào. Agent ghi intake và hỏi phần còn thiếu.", "Khi đồ thị đã chạy được, bắt đầu Goal ở thanh hội thoại. Một lần chạy xong chưa phải Goal hoàn thành; bạn bấm hoàn thành hoặc xóa. Mở hội thoại không tự tạo Goal.", "«Bắt đầu hội thoại» mở panel Agent. «Chỉ tạo canvas» vào workbench ở tab chi tiết. Chỉ có tên thì trước hết có node thông tin sản phẩm.", "Chạy node hệ thống hình ảnh, brief sáng tạo và prompt để ghi nội dung, rồi mới chạy node ảnh. Các node không nối với nhau chạy cùng lúc; một ảnh thất bại không dừng các shot cùng lớp. Chạy node nội dung không xuất hình.", "Chỉnh prompt, thông số tạo và liên kết ảnh tham chiếu. Ảnh tải lên và kết quả tạo thuộc thư viện sản phẩm; tài nguyên dài hạn xuyên sản phẩm nằm ở `/media-library`."] }] },
