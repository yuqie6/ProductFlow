...
const HELP_DOCS = {
          ...
          blocks: [
            {
              ...
              items: [
                ...
                "画面方案：按目标、构图与场景、重点内容、文字组织文稿，设置文字模式和语种。运行该节点不会生图。无文字模式保留已写标题，执行时不带文字。",
                "图片生成：查看结果，调整本图差异、比例、分辨率、质量、背景和参考保真度。未覆盖字段继承方案；修改本图只保存修改项，恢复继承后跟随方案新值。文字可在本图独立设置。运行只出图，不重写共享文稿。交付尺寸和格式单独设置，不调用模型。",
              ],
            },
          ],
        },
  ...
  "en-US": [
    ...
    },
    {
      ...
      icon: GitBranch,
      sections: [
        { id: "nodes", title: "Node responsibilities", blocks: [{ type: "list", items: ["Product facts separate confirmed facts from pending observations. Pending or conflicting facts are excluded from generation until confirmed.", "A reference image binds one product-library image with a role and note: product identity, environment, style or evidence.", "Creative brief holds the goal, key messages, required elements, prohibitions and missing facts.", "Series style holds shared style and an editable palette with color roles and notes.", "Picture plans organize the goal, composition, scene, content and text, with advanced details collapsed. The plan owns text mode and language. Running it does not render images.", "Image generation shows the result and stores only per-picture changes, optional text replacement and model settings. Restoring inheritance removes the local change and follows the plan's latest value. Delivery settings are separate. Running this node only renders from the live prompt document; a prompt artifact is not required. A reference edge is optional. Authored content nodes are not overwritten on a full-graph run. Direct create wires uploads to visual, brief, prompt, and image nodes."] }] },
  ...
  "vi-VN": [
    ...
    },
    {
      ...
      icon: GitBranch,
      sections: [
        { id: "nodes", title: "Vai trò của node", blocks: [{ type: "list", items: ["Thông tin sản phẩm tách dữ kiện đã xác nhận và thông tin chờ xác nhận. Thông tin chưa xác nhận hoặc mâu thuẫn không được dùng để tạo ảnh.", "Ảnh tham chiếu giữ một ảnh thư viện cùng vai trò và ghi chú: sản phẩm, bối cảnh, phong cách hoặc bằng chứng.", "Brief sáng tạo gồm mục tiêu, thông điệp, yếu tố bắt buộc, điều cấm và dữ kiện còn thiếu.", "Phong cách bộ ảnh gồm phong cách chung và bảng màu có mục đích, ghi chú và ô chọn màu.", "Phương án ảnh tổ chức mục tiêu, bố cục, bối cảnh, nội dung và chữ; quản lý chế độ chữ và ngôn ngữ. Chi tiết nâng cao được thu gọn. Chạy node này không tạo ảnh.", "Node tạo ảnh hiển thị kết quả, lưu thay đổi riêng, chữ thay thế và cài đặt mô hình. Khôi phục kế thừa xóa thay đổi riêng và dùng giá trị mới nhất của phương án. Cài đặt bàn giao được tách riêng. Chạy node này chỉ xuất hình từ văn bản prompt hiện tại, không cần artifact. Cạnh tham chiếu là tùy chọn. Node nội dung đã viết không bị chạy cả đồ thị ghi đè. Tạo trực tiếp nối ảnh tải lên vào hệ thống hình ảnh, brief, prompt và node ảnh."] }] },
