  ...
  for (const locale of LOCALES) {
    test(`email registration ${width} ${locale}`, async ({ page }, testInfo) => {
      ...
      await page.route("**/api/auth/session", (route) => route.fulfill({ json: {
        ...
        ...(signedIn ? { user: { id: "user", email: "member@example.com", display_name: "Member", is_operator: false }, memberships: [{ merchant_id: "merchant", merchant_name: "Shop", role: "owner", status: "active", merchant_status: "active" }] } : {}),
      } }));
