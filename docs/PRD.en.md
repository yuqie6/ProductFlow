...
The project has no formal commercial release or commercial merchant users yet. The development mainline accepts breaking changes and may require recreating the database and storage. It maintains one schema-v3 graph, API, and execution model. The current development bootstrap still creates the only development merchant; public registration is implemented and has passed real-browser and real SMTP/IMAP mail-receipt verification. Personal profile, password recovery and session management are implemented; account preferences are implemented and real payments remain pending; administrator product management pages and current-operation records are implemented; quota and stable-release recovery/upgrade mechanisms have scoped evidence.

The intended product is a self-hostable, multi-merchant SaaS, also operated by the project owner as a service using the same product. Each ordinary account owns one merchant. Workspace switching and team UX are excluded; administrators have explicit merchant-targeted product read, facts-edit and delete pages with operation records; the [release roadmap](ROADMAP.en.md) owns the planned isolation, experience, competitive and deployment requirements. This PRD describes connected capabilities only.
...
Account and navigation controls share saved interface language/theme preferences; product output language stays unchanged. Account holders can rename their own merchant. Merchant suspension prevents renaming but leaves personal profile, preferences, password changes and session revocation available.

Administrators use `/ops` to find merchants and manage each merchant’s products through explicit target routes. Product facts retain version checks; deletion retains business restrictions. Task records are read-only. Quota adjustments use the existing idempotent ledger. Operator action records retain actor, target, time and confirmed or uncertain outcomes. Products retain their merchant ownership.
