...
The domain harness is a versioned artifact under `agent-service/harness/`. `src/harness.ts` loads four behavior instruction sections, a pinned policy digest and empty overlay/runtime-control placeholders, then computes SHA-256 over the existing canonical JSON representation. `pi-runtime.ts` freezes the object at module startup and passes its instructions to Pi. Authority and confirmation remain in the bundled `go/prompts/agent/runtime-policy.md`; mismatched digests and undeclared fields fail loading. Production attribution, candidate evolution and hot promotion are not implemented. Tests: `harness/harness.test.ts` and `src/pi-runtime-harness.test.ts`.
