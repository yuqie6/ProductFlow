...
ImageSession recovery discovers candidates in `created_at/id` order using `FOR UPDATE SKIP LOCKED` to bypass a locked prefix. Discovery returns at most 26 candidate IDs by default (25 processing items and one probe), briefly locking rows until its transaction ends. State transitions recheck each task in a separate transaction; discovery locks are not held across the recovery batch. `HasMore` describes eligible unlocked candidates beyond the batch limit, not total database backlog. Implementation and regression: `go/internal/imagesession/recovery.go`, `recovery_test.go`.
