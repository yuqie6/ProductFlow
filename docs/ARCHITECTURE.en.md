...
- Frontend: Vitest, ESLint, TypeScript, and Vite production build. The skip-Agent full-graph browser gate against real prompt/image providers is opt-in: `just web-e2e-live-graph`. Bounded document-authority action search runs with `just go-test` (`go/internal/graph/authority_search_test.go`); deeper search is `just go-test-canvas-search`. The idle Inspector rewrite/candidate browser gate against mock providers is opt-in: `just web-e2e-canvas-document`. Typing in the inspector while a cook is running is not in that gate yet; see [`audits/tasks/canvas-inspector-midrun.md`](audits/tasks/canvas-inspector-midrun.md). Acceptance: [`audits/canvas-test-system.md`](audits/canvas-test-system.md).
