...
Delivery adoption snapshots persist in `delivery_adoption_versions` / `delivery_adoption_slots`: they reference source asset ids, slot purpose, sort order, DeliverySpec, and optional fact/visual versions without copying image bytes. Explicit adopt creates a new immutable version and points `products.current_delivery_adoption_version_id` at it; node re-runs do not mutate prior versions. Slots with `quality_status=fail` or text overflow cannot enter the qualified adoption set (IQ-CF-08). Preview and export share one filename plan and reuse existing DeliveryRenditionJob / ZIP paths. HTTP: `/api/v3/products/{id}/delivery-adoptions` plus `current` / `{version_id}` / `preview` / `renditions` / `export`. Document candidate adopt (O1–O7) remains separate.
