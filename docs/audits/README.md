...
| 工作流体验 | [canvas-test-system.md](canvas-test-system.md) | 交付编辑、运行、撤销与结果使用的完整行为；已有文稿权威任务完成，当前无待执行单 | 复现商家操作 → 必要前后端修复 → 浏览器与业务状态验收 | 当前 Graph/资产合同与固定 provider；不为等待模型涨分而阻塞确定性修复 |
