...
|---|---|---|---|---|
| 商家平台 | [merchant-platform.md](merchant-platform.md) | 注册、隔离与额度已有局部证据；完整账号/管理员流程未齐；R5 未通过 | 冻结全链路覆盖与实施批次 → 账号与自有 Merchant 归属 → 业务/异步/Agent 隔离 → 商业额度与运营权限 → 管理员商品管理与隔离验收 | 完整隔离验收前不开放互不信任的第二商家；额度入口接线与 R5 仍开放 |
...
| 工作流体验 | [canvas-test-system.md](canvas-test-system.md) | 承担从首次套图、指定修改、选定下载到下一商品复用的完整体验；已有配方与局部编辑不重复立项 | 完整成果投影 → 操作走查 → 采用/交付快照 → 品牌复用交互 → 整条任务验收 | 当前 Graph/资产/运行合同可用；新视图先验证，批量扩展不预先设为核心前置 |
