...
|---|---|---|---|---|
| 工作流体验 | [canvas-test-system.md](canvas-test-system.md) | 开工 | 商家编辑、运行与结果使用链的业务正确性；当前交付聚焦文稿权威与检查器草稿保存 | 文稿权威 C0–C6 与 USER_GUIDE §3.4 检查器运行/到此/取消浏览器证据均已归档；本组无未关闭 issue，不重复派版本语义任务 |
...
| 平台可靠性 | [performance-governance.md](performance-governance.md) | 开工 | 执行不丢、不重复、不越权；lease、journal、恢复、队列、容量与查询成本 | 入队误占 admission 已归档；PERF-12 仍缺目标规模 payload，本轮不发该后续单；journal 调查结论为保留现状，不发实现单；G-06 汇总评测证据，G-07 按候选基线复核 |
