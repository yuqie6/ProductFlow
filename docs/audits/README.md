...
| 工作流体验 | [canvas-test-system.md](canvas-test-system.md) | 负责商家从要求与参考到可用图片的操作体验；分别记录实现、证据与未知，任务状态见看板 | 复现操作问题 → 必要跨层修复 → 核实际业务结果与操作负担 | 当前 Graph/资产合同与固定 provider；文稿权威已有交付，资产与配方验收仍有开放任务 |
