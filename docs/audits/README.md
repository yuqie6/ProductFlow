...
| 平台可靠性 | [performance-governance.md](performance-governance.md) | 交付可验证的执行正确性、恢复与资源成本；当前无待执行单，目标负载证据仍有缺口 | 复现运行或负载问题 → 完整执行链修复 → 故障/容量验证 | 固定业务合同与专属运行环境；全产品发布 Gate 汇总不构成本组每个任务的完成条件 |
