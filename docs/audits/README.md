...
| 工作流体验 | [canvas-test-system.md](canvas-test-system.md) | 负责商家从要求与参考到可用图片的操作体验；分别记录实现、证据与未知，任务状态见看板 | 复现操作问题 → 必要跨层修复 → 核实际业务结果与操作负担 | 当前 Graph/资产合同与固定 provider；资产、片段、完整配方与局部编辑已有证据；商家任务耗时与镜头列表主界面仍未知 |
