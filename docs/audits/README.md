...
| 平台可靠性 | [performance-governance.md](performance-governance.md) | 开工 | 执行不丢、不重复、不越权；lease、journal、恢复、队列、容量与查询成本 | Graph 自动采用并发已归档；PERF-12 仍缺目标规模 payload，本轮不发该后续单；journal 调查结论为保留现状，不发实现单；G-06 汇总评测证据，G-07 按候选基线复核 |
