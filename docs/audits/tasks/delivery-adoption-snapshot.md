...
`go/internal/delivery/`、ZIP/规格转换；节点 `current_artifact_id` / 历史候选；成果视图入口（[archive/delivery-workbench-projection](archive/delivery-workbench-projection.md)）。文稿候选采用（O1–O7）≠ 交付采用，须分开建模。
...
- 消费图片质量 IQ-CF-08 合格判据：不合格图可生成但不得进入已采用交付合格集。
...
- 原因：无（成果投影已归档）。
- 解除条件：无。
- 跟进者：sub-delivery/delivery-adoption-snapshot。
- 交接：已认领；未改代码。
