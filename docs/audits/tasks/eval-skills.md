...
- 原因：图观察已交付；旧 A 不可跨题集比较。本单暂不恢复付费 A/B。
- 解除条件：[图观察权威](archive/eval-graph-observation-authority.md) 已交付；由维护者确认新共同版本和独占 A/B 窗口后恢复。不能拿旧 A 跨题集比较或自动接受候选。
- 跟进者：维护者在图观察交付后安排复验窗口。
- 未完成：候选两次 k=3、稳定性、系统性失败改善及 mutate kill_rate。regression 与 Self-Harness G1 / G2 均未通过，不把本候选提交写成任务完成。
- 交接：2026-09-06T17:40:00+08:00 用户确认原执行会话已关闭。协调者检查无本单运行进程后释放认领占用。A 产物与已提交 Skill 候选保留，不改共享 DB / provider。
