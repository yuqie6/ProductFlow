...
[商家成图闭环](delivery-merchant-image-loop-gate.md) 本门 PASS，但检查器「确认事实」UI 触发 `Cannot read properties of null (reading 'length')`；采证改走 `PUT /facts`。商家日常改稿应能点确认，不能只靠 API 绕过。
...
| 命令 | 日期 | 结果 |
|---|---|---|
| `pnpm --dir web exec vitest run …factImpactPreview.test.ts …graphNodeEditorDrafts.test.ts` | 2026-09-07 | **10 passed**（维护者复跑 PASS） |
| `just docs-check` | 2026-09-07 | PASS |

- 交付定位：随本任务提交
- 审核者 / 结论：维护者通过（2026-09-07）；前端兜底正确；≠R2
