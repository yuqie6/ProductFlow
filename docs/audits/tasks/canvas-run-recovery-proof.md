...
[完整操作链核查](archive/canvas-workflow-coverage.md) 发现：场景运行仅有 request builder 与后端 selection 测试；`workbench-v3-actions.spec.ts` 的 can retry 用例只检查按钮可见。用户要求本组完整落地，需要证明点击之后的真实业务结果。
