...
- 当前前置：River 代码与本地开发切换已交付，具备重新固定候选采证的条件；任务恢复开放，尚未重新认领。
- 执行条件：认领时冻结最终候选、独立资源及测量器；无需真实模型凭据。
- 跟进者：开发协调者。
- 交接：已保留 e190 及后续公平性采证；完整混合负载、故障窗口和 commit-to-SSE 缺口见下文。当前无采证进程或冻结资源。

## 证据

- 已有命令、日期和结果见下方分轮记录；新 River 完整批次尚未执行。
...
## 2026-09-09 队列切换期间暂停采证

root 接管统一队列实现，当前没有运行本容量任务的采证进程或冻结新候选。旧独立产物与脚本保留，不占候选工作树；释放认领。新队列的故障/公平局部门由实现任务持有，完整容量合同不随之放宽。统一队列候选交付后由 root 重新冻结完整容量采证版本。

2026-09-09 本地开发 River 切换完成，旧停止信封不再是前置阻塞。本任务恢复开放；已有公平局部门通过不替代完整混合负载和 SSE/故障合同，也不要求立刻扩大为生产容量承诺。
