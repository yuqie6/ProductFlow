...
现阶段：B10/MP-B 与总纲 **R1 已裁定通过**（夹具双商口径）；**在运营明确邀请前仍不得对互不信任的第二商家开放产品注册**（`CreateMerchant` 保持 409）。[总纲 R2/R5 对照刷新](tasks/archive/roadmap-r2-r5-refresh.md) 已归档（两门仍未通过）。[新商家额度引导](tasks/archive/merchant-mp-c-bootstrap-quota.md) 已交付（`QUOTA_TRIAL_UNITS`；≠R5）。
...
| 类别 | 命令要点 | 计数 |
|---|---|---|
| HTTP 注册 | `go/cmd/productflow-api/register.go` 挂载含 `brand`（及既有 auth/settings/product/…/visualsystem/localedit/agent） | **业务路由 222**：相对冻结枚举 214 + Brand B0 的 4 条 + 价格目录 2 条 + Brand B1 的 2 条（`GET/PUT /api/v3/products/:id/brand-selection`）；其余分区仍以 register.go 为准 |
