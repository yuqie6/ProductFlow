...
| MP-B 隔离 | A/B 商家合法操作成功，所有交叉读写、导出、事件、Agent 与后台路径拒绝；查询与引用一致性约束有测试 | **B1–B7 已交付**（根归属、商品链、Graph/配方、图库绑定、会话生图、交付/局部编辑、Agent 工具链，跨商 404）；队列/前端见 [B8](tasks/merchant-queue-frontend.md)；完整双商门未过 |
...
| **B8** | 队列与前端 | 矩阵 J\*：五 Actor 快照、Restage、recovery；前端 merchant 前缀与 SSE 取消 | 混合队列下本商 UI 正确 | 改信封商家无效；旧订阅；切换迟到 | 单商（实现见 [merchant-queue-frontend](tasks/merchant-queue-frontend.md)） |
...
- 业务根表 `merchant_id` 与查询过滤：**B1 已交付**（跨商 404）；**B2 商品子链/下载/ZIP/workspace/from-recipe 已交付**；**B3 Graph/SSE/recipe apply 已交付**；**B4 图库绑定已交付**；**B5 会话生图已交付**；**B6 交付/局部编辑已交付**；**B7 Agent 工具链已交付**；队列/前端见 [B8](tasks/merchant-queue-frontend.md)。
- 双商自动化套件与前端切换套件未建。
- MP-C/MP-D 未开始。
