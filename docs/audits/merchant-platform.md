...
|---|---|---|
| 本人资料、密码、会话 | 本人 | 本人 |
| 商品、素材、生产与交付 | 自有商家 | 可管理各商家商品，生成消费不由管理权限自动授予 |
...
个人语言/主题已按本人账号持久化，顶栏与账户页使用同一保存入口；商品输出语种独立。商家名称属于自有商家；provider/密钥与运行配置属于站点管理员。复用现有字段和页面，不为低影响偏好默认加团队并发协作协议。

`/account` 和 `/password-recovery` 已实现；`/ops`、`/ops/merchants/:merchantId`、`/ops/merchants/:merchantId/products/:productId` 已实现商家目录与目标商品管理、任务/额度记录和调账。`/merchant/usage` 仍是待实现目标；`/settings` 已有管理员保护。运营界面能够展示目标商品与所属商家，不以仅可读元数据或未实现支持授权阻塞。列表分页有界，操作日志记录实际管理员、目标、时间与结果，不能代替业务账本。
...
- 已实现：公开注册/SMTP、直接账号归属、本人资料、密码修改/恢复、有效会话分页与撤销、商品跨账号隔离基础、额度账本、管理员站点配置及显式目标商家管理页面。
- 已实现商品入口的商家概览与继续工作；详情和证据见 [商家概览交付](tasks/archive/saas-merchant-dashboard.md)。待实现：普通商家用量，以及当前候选的完整授权与事件失效复验。
- [本人偏好与商家资料](tasks/archive/saas-preferences-settings.md) 已通过新建隔离库的完整 auth、迁移与真实 API/浏览器验收：跨浏览器恢复、匿名/账号隔离、停用商家仍可管理个人安全；迟到账户操作不会改写新账号会话。
- [管理员管理页面](tasks/archive/saas-ops-console.md) 已通过隔离双商家/独立管理员 Go/PG 与真实浏览器验收；资料冲突、审计失败事务回滚、媒体归属和幂等调账有回归。商品操作记录只覆盖新受理修改/删除，未知结果保留，额度沿原账本。该交付不代表完整 SaaS 或新图片质量通过。
...
统一后台队列代码已交付，五类任务在同一事务持久化 River 作业，领取时核对商家与执行轮。隔离公平、故障和迁移证据见 [统一队列验收](tasks/archive/pg-queue-integration.md)。本地开发服务现已切换 River，账号、商品和素材保留；开发切换证据见平台可靠性章程。
