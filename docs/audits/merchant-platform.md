...
普通账号登录后直接使用自己的商品；服务端明确唯一归属。[身份收敛](tasks/archive/saas-identity-convergence.md) 已交付，成员表、团队角色与切换读写者已移除；迁移遇到旧归属歧义时失败并保留数据。个人账户不依赖商家选择或团队管理；密码恢复尚未实现，不能展示假成功或以管理员转发邀请代替。
