...
2026-09-07 正式版职责校准：本组负责 [总纲第 5 节](../ROADMAP.md#5-正常体验的最低要求) 的创建、成果浏览、修图、采用、交付和复用体验。先交付 [完整成果投影](tasks/delivery-workbench-projection.md)，保留现有画布默认与手动入口；是否切换默认由任务走查证据决定。后续采用/交付快照和品牌复用是新增合同，基础配方 preview/原子确认、局部编辑和 ZIP 已有实现，不重复发布。
