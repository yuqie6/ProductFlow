...
- IQ-CF-08 提供身份、文字和路线声明的质量判定。用户于 2026-09-08 明确：检查失败或无法判断时显示问题，允许用户确认后采用及下载；保存 fail/unchecked，不以确认改写质量结论。旧硬闸证据只反映当时实现；资产完整性与权限仍不可绕过。
...
当前开发按 [路线图](../ROADMAP.md) 处理小范围开源使用体验。简单操作与可选高级控制共用同一业务对象；每项功能检查入口、数据来源与结果去向。任务内已授权操作连续完成，确认边界按覆盖、删除、范围扩大等实际影响判断。外部反馈作为补充，内部完整任务与浏览器证据足以指导修复，但不冒充用户满意度。

结果卡现有明确预览入口，采用版本与当前结果未对应时展示原因，导出保留已采用快照；长差异列表不挤占成果区。四语言桌面/窄屏连续操作验证见 [工作台衔接交付](tasks/archive/workbench-continuity.md)，浏览器使用路由 mock，不代表服务端 ZIP 验收。
...
just web-e2e-delivery-merchant-image-loop
