...
| P3 | Miner | `缺失` | 封闭失败签名 φ；仅开发集 L1 trials → 证据包；簇抽读合格才给提案器 | 自动改文件；读取隐藏回归或独立验收轨迹 | φ 词表见下节；前置 [eval-collection-isolation](tasks/eval-collection-isolation.md) 未完成，尚不发布 Miner 实现 |
