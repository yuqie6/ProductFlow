...
**不要从本文件开工。** 未完成工作：[`tasks/perf-imagesession-detail.md`](tasks/perf-imagesession-detail.md)、[`tasks/perf-dispatcher-latency.md`](tasks/perf-dispatcher-latency.md)、[`tasks/perf-capacity-metrics.md`](tasks/perf-capacity-metrics.md)。GraphRun 摘要/详情、SSE fanout、recovery 有界批次已经接线。
...
| PERF-06 | dispatcher recovery 拖慢投递 | 部分完成 | dispatch loop 与 recovery cadence 解耦；watch 默认每秒投递、每 10 秒 recovery；`Stage`/`resetPending`/`MarkFailed` 回 PENDING/`ReleaseForRetry` 在同一事务 `NOTIFY productflow_dispatch`；标 DEAD 不通知；陈旧 SENT 对账每轮最多 100 条。`TestReplicaFieldTwoDispatchersClaimOnce`：两 dispatcher 同时 `RunDispatcherOnce`，8 条 PENDING 各入队一次。`TestReplicaFieldDispatcherSIGKILLSurvivor`：IsolatedMigrated + Redis DB 14 上两个真实 `productflow-dispatcher --watch`，SIGKILL 副本 A 后副本 B 仍把新 PENDING 标 SENT（2026-09-05 `just go-test-staging-field`，9.39s） | 退避未到期时唤醒仍会跳过该行，继续依赖 ticker；缺负载下 dispatch latency；`just staging-up` 杀容器未跑（会与 `just dev` 争 Postgres 15432） |
