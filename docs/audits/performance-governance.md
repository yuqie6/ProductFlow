...
| PERF-06 | dispatcher recovery 拖慢投递 | 部分完成 | dispatch loop 与 recovery cadence 解耦；watch 默认每秒投递、每 10 秒 recovery；`Stage`/`resetPending` 在同一事务 `NOTIFY productflow_dispatch`；陈旧 SENT 对账每轮最多 100 条 | `MarkFailed`/`ReleaseForRetry` 拉回 PENDING 仍不通知，可能只等 ticker；缺负载下 dispatch latency |
