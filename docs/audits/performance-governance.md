...
连续生图批次检查点后的进程退出已有 [跨进程回归](../history/agent-runtime-timeline.md#2026-09-06-连续生图检查点退出与取消恢复)：lease 有效时不重投，过期后正常路径只补第二张，已取消路径不再调用 provider；首张真实下载字节保持。此窗口的恢复等待由消费 lease（默认 claim 后 35 分钟）和 SENT 对账阈值共同决定，不是瞬时续投，也不走 running 的 90 分钟闲置判断。测试推进独立库时间戳，没有实等 35 分钟。
