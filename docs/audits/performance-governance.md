...
**平台可靠性组章程。执行以已发布 issue 为界。** 合并原性能与生产可靠性职责，接收 journal 职责调查。当前任务、认领和阻塞见 [Issue 看板](tasks/README.md)。GraphRun 摘要/详情、SSE fanout、recovery 有界批次已经接线。详情、时延、容量指标、dispatcher 积压续投、Graph 自动采用并发与连续生图入队 admission 已经归档。[perf-dispatcher-backlog](tasks/archive/perf-dispatcher-backlog.md) 后本地 500 条突发单副本 PENDING→SENT p95 为 0.438s。[perf-imagesession-enqueue-admission](tasks/archive/perf-imagesession-enqueue-admission.md) 后入队不再持 capacity advisory 或误计 denied。PERF-12 仍缺目标规模 payload，本轮不发该后续单。本地 p95 不表示生产 SLO。本组当前无未关闭 issue。
...
| PERF-04 | 生图容量锁顺序 | 部分完成 | Graph 为 `capacity advisory -> run -> node`；ImageSession claim 为 `capacity advisory -> task`，取得 task 锁后重新核对状态。`TestReplicaFieldTwoWorkersRespectGenerationCapacity`：上限 1 时两 worker 同时 claim，1 running / 1 waiting_for_capacity。[perf-imagesession-enqueue-admission](tasks/archive/perf-imagesession-enqueue-admission.md)：`Generate` 入队不再调用 `GenerationCapacityAvailable`；满容量时 queued 仍写入、denied 不变，claim 才 later | 全库单钥匙和 noisy neighbor 仍存在；SaaS 前需按 workspace/tenant 重构 |
...
| PERF-08 | ImageSession 列表 N+1 | 部分完成 | 列表批量摘要、首屏 history 与 `GET /history` keyset 保留。详情三组任务各 LIMIT 20，去重最多 60；队列排名只返回所需 ID。[详情修复](tasks/archive/perf-imagesession-detail.md) 与 [HTTP 负载验收](tasks/archive/perf-imagesession-http-load.md)：隔离 25k 会话 / 10k 轮次 / 1k 任务，七条 HTTP 读取各 100 样本通过；热会话详情 50 条任务、258,037B、p95 16.33ms，列表 page 20/100 p95 25.54/25.46ms，history page 100 为 302,625B、p95 7.33ms | COUNT 与首屏关联任务仍 Seq Scan；HTTP 为单热会话、单客户端、元数据读取，生产访问分布、并发与图片下载未验，不把返回有界等同于扫描成本有界 |
...
状态：dispatch/recovery cadence、Agent recovery 事务、全部 recovery owner 的有界候选批次、dispatcher `has_more`/duration/error 日志、dispatcher 各域 recovery histogram、候选锁查询耗时、API 当前 PostgreSQL 锁等待和 queued/stale-running backlog 指标已落地；capacity wait/running/denied 指标已落地；ImageSession HTTP 入队不再持 generation advisory；业务域 recovery 已按单聚合状态迁移与 restage 拆事务；watch dispatch 在 claim 满批时立即续跑，同一轮 SENT+enqueue 有界并发。本地 500 条突发 PENDING→SENT p95 已低于 1s。生产 SLO、重 recovery 负载和 `just staging-up` 杀容器仍观察。
...
4. 单商家阶段继续用 PostgreSQL capacity advisory 保证准确 admission；capacity wait 用 `productflow_advisory_lock_wait_seconds`，running/denied 用 `productflow_generation_admission_*`。ImageSession `Generate` 只写 queued，admission 在 claim。SaaS 前替换为按 workspace/tenant 的 admission token；PostgreSQL 只做对账。
...
- ImageSession HTTP：`just go-test-imagesession-http-load` 以隔离迁移库和带管理员 cookie 的真实 HTTP 验证列表、详情与历史续页；25k 会话 / 10k 轮次 / 1k 任务，每路径 10 次预热和 100 次测量，记录 p50/p95 与最大 payload。列表和历史页采用建议初始预算 p95 <300ms，全部响应在固定 fixture 下 <1MiB；详情延迟仅观测。该门不调用 provider，也不验证媒体下载或生产并发。
...
| 2026-09-05 | 连续生图目标规模 HTTP 验收入口 | [归档任务](tasks/archive/perf-imagesession-http-load.md)：复用隔离 SQL fixture，增加 2160B 提示词、512B 进度、990 条 effect、6 条参考图元数据；七条读取各 100 样本通过，列表 page 20/100 p95 25.54/25.46ms，详情 16.33ms / 258,037B，history page 100 为 7.33ms / 302,625B。包回归 6.756s，原 query-plan 2.400s 通过 | 未改运行时、没有性能提升声明；单客户端 loopback HTTP、单热会话、只读元数据，COUNT/关联扫描与生产负载仍观察 |
...
| 2026-09-05 | 连续生图入队不再占用 admission，见[归档任务](tasks/archive/perf-imagesession-enqueue-admission.md) | `TestGenerateWhenCapacityFullStillQueuesWithoutDenied` 先红（入队 denied 0→1）后绿；claim 满容量才 denied+1。包回归 9.096s | 全库单钥匙仍在；未跑 tenant 分钥匙 |
