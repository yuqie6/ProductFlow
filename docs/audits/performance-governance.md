...
| PERF-13 | Agent journal batch 写入 P95 | 部分完成 | `AppendEvents` 将 batch 内已有 sequence 从逐条 `Take` 改为一次查询，保留 projection lock、幂等 replay 和逐条 fold；历史容量 gate P95=240ms/211ms，2026-09-04 当前 HEAD 复核 P95=232.800884ms，100 SSE 通过 | 需要持续 batch histogram、目标规模负载与锁等待观测 |
