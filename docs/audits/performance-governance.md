...
| PERF-05 | Agent 与业务 recovery 长事务 | 部分完成 | Agent 的过期 execution 回收、queued Task reserve、projection 查询、outbox restage 已分阶段提交；所有 recovery owner 默认每轮最多 25 条，业务域使用 `SKIP LOCKED`，queued 行过滤活跃 outbox；dispatcher 记录 recovery duration/error，dispatcher metrics 提供各域 duration/candidate lock query histogram，API metrics 提供 queued 与 stale-running backlog 和当前锁等待 | 各域仍在一个有界事务内处理一批聚合；仍需拆分批次内状态迁移与 restage，并用目标规模确认锁等待分布 |
...
| PERF-08 | ImageSession 列表 N+1 | 部分完成 | 最新轮次与资产、round count 改成批量查询；`image_sessions(updated_at DESC,id DESC)` 索引已写入并完成本地迁移；列表已同步 Go/Web 游标分页与 load-more | 详情页任务/effect/队列总览仍有多次查询；需要真实规模 query plan 与 payload 记录 |
...
| PERF-13 | Agent journal batch 写入 P95 | 部分完成 | `AppendEvents` 将 batch 内已有 sequence 从逐条 `Take` 改为一次查询，保留 projection lock、幂等 replay 和逐条 fold；容量 gate 连续两次通过，P95=240ms/211ms，100 SSE 通过 | 需要持续 batch histogram、目标规模负载与锁等待观测 |
...
当前工作树已经完成 GraphRun 列表的摘要/详情合同拆分、Graph projection 的批量读取和 Graph SSE 的状态读取边界。修复后 Go HTTP gate、目标规模摘要/详情 query plan、浏览器冷/热启动、重复请求、按需详情打开和 Web bundle budget 均已通过；生产详情打开率与跨副本容量仍属于部署后观察项，因此本节把代码验收与运行观测分开记录。
...
三层 gate 已落地并在当前工作树通过。它们是本地回归证据，不替代生产 SLO；每次重新跑 gate 都要记录 fixture、服务 commit 和环境。
...
2. HTTP gate 与 before/after：正式报告为 `/tmp/productflow-http-gate-head-current.json` 和 `/tmp/productflow-http-gate-pre-split-current.json`，均为 warmup=20、每路径 400 样本、固定 fixture `product=18470ef4-d41a-4327-986f-4d9588198946`、`graph=a98aa8a2-87c1-4312-96a8-0c628b00857a`。当前实现的 gate：summary p50/p95/p99=3.46/4.20/4.48ms、最大 payload=15,629B；detail=3.74/4.54/5.10ms、最大 payload=2,623B；current→runs 串行 p95=12.45ms；商品列表 page 20/100 串行 p95=3.40/4.02ms，20 并发 p95=12.03ms；所有请求 400/400 且字段合同通过。

   - 本次改动净对照（`HEAD=6726a490` -> 当前 dirty，同一路径随机交错）：`current` p95=10.32 -> 9.73ms（-5.7%），`runs` p95=4.44 -> 4.36ms（-1.8%）；两条响应 payload 均保持 34,514B 和 15,629B。这证明本次改动没有造成 API 回归，HTTP 毫秒收益在小 fixture 上有限。
   - 完整读路径对照（`2d9bba87`，GraphRun 摘要/详情拆分前 -> 当前 dirty）：`current` p95=9.94 -> 9.32ms（-6.2%），payload 均 34,514B；`runs` p95=16.78 -> 4.67ms（-72.2%），payload=67,705 -> 15,629B（-76.9%）。后一个大收益包含已在 HEAD 中的 GraphRun 摘要/详情拆分，不能全部归因于本次 dirty diff。
...
3. Playwright：`web/e2e/workbench-performance.spec.ts` 记录冷/热启动、`current`、`runs`、Graph SSE、rich detail 和显式详情动作。当前 fixture 结果为 cold/warm TTI=2,170/2,044ms，current/runs 各 1 次，初始 rich detail=0，Graph SSE=0（fixture 没有进行中 run），节点检查器显式展开 0 次额外请求，运行历史显式打开 1 次详情请求，19 条可见 run。脚本对预期的 Agent bootstrap 409 做白名单处理，并检查其它 console/page/network/HTTP error；在 default 3,000ms TTI budget 下通过。
...
| GRAPH-READ-03 | 修复后 HTTP p95/payload 与同 Go 版本 A/B | `scripts/bench_workbench_http.py`、`justfile` | `/tmp/productflow-http-gate-head-current.json`：当前 summary/detail p95=4.20/4.54ms，最大 payload=15,629/2,623B；同一 fixture `HEAD -> dirty` 随机交错：current p95=10.32/9.73ms，runs p95=4.44/4.36ms；`/tmp/productflow-http-gate-pre-split-current.json`：拆分前 runs p95=16.78ms、67,705B -> 当前 4.67ms、15,629B；所有样本 400/400 | 旧 Python A/B 未启用；生产详情打开率和跨副本容量仍观察 |
...
| GRAPH-READ-05 | 浏览器 TTI、重复请求、SSE 连接数和 bundle 成本 | `web/src/pages/workbench`、Web e2e、`platform/metrics` | `just web-e2e-workbench-performance` 已过：cold/warm=2,170/2,044ms，current/runs 各 1，初始 detail=0，显式 run detail=1；Graph SSE 当前 fixture=0；预期 Agent bootstrap 409 已白名单，其它 HTTP/page/network failure=0；bundle budget 已接入 `just web-build`；跨副本汇总仍需部署观测 |
