...
11. [subject_preserve 声明诚实](tasks/archive/image-quality-subject-preserve-honesty.md)：已归档；生成式出图时不得 appearance_may_change=false；≠R3。
12. [合成 PNG 作交付字节](tasks/archive/image-quality-subject-compose-deliver.md)：已归档；compose Pass → 合成 PNG 交付 + `SubjectPreserveDeliveryFromCompose`（跳过 GenerateImage）；失败可存生成式但不合格；≠R3。
13. 候选不得同时改评委或金标；评分合同缺陷另立先行任务。
...
| 视觉方案 | 表 `visual_systems` / `visual_system_versions` / `product_visual_selections`；`ResolveInheritance` 四级优先级；节点 `visual_overlay`；`mergeImageVisual`；配方 `preferred_visual_system_version_id` + `reuse_preview`；追加版本 Impact + 显式采用 | Brand 表 B0 已建（`brands` + CRUD）；继承占位为 `brand_not_selected` / `brand_exists_no_style_merge`（≠品牌色全量合并；≠跨商分享） |
...
| IQ-CF-02 | **图位文字追溯**：信息图成稿中的可核验文字（规格、容量、材质、卖点短句）须能追到本商品 fact key 或显式「用户本图覆盖」标记；卖点图默认一图一主要购买理由。无依据文字的质量判定不得为合格；采用/交付允许用户明确确认，不改变质量判定。 | 节点/产物元数据或导出旁路索引；对照 `fact_keys` 与成片 OCR/人工检；内容策略候选可作输入不得替代本闸 | `部分完成`（CF-B1 元数据+卖点检查器；采用硬闸已接 `text_qualified`；[OCR B0](tasks/archive/image-quality-ocr-trace-b0.md)；[采用 OCR 接线](tasks/archive/image-quality-ocr-adoption-wire.md)；[硬闸收窄](tasks/archive/image-quality-ocr-adoption-scope.md)：有期望则对照期望字，**不以残余墨迹拦商品成片**；CJK/开放词表/生成路径自动 Apply/R3 未宣称） |
...
| IQ-CF-08 | **与采用快照交接**：本组判定「身份合格 / 文字合格 / 路线声明正确」作为质量状态；体验组 [delivery-adoption-snapshot](tasks/archive/delivery-adoption-snapshot.md) 负责采用动作、快照持久化、导出 UX。质量不合格或无法判断时，展示问题并允许用户确认后采用及下载；保存原检查状态，不将用户确认算作检查通过。O1–O7 文稿采用 ≠ 交付采用。 | 跨组合同引用；本组提供质量判据表，不写采用表模型 | `部分完成`（现有实现与冻结证据见 [compete-adoption-hard-gate](tasks/archive/compete-adoption-hard-gate.md)：`CreateAdoption` 强制 `text_qualified`/`route_qualified`；无元数据不得 `pass`；该历史硬闸已由用户确认采用政策替代，现行行为见 [政策纠偏](tasks/archive/saas-policy-alignment.md)；[采用 OCR 接线](tasks/archive/image-quality-ocr-adoption-wire.md) 默认消费成片对照；主体提取/R3 另列） |

#### IQ-CF-06 技术选型比较维度（实现任务须落备忘）

实现切片启动前，对候选渲染/编辑组件（含自研最小集）逐项记分，**未经验证不得写入依赖**。CF-B4 选型结论见 [iq-cf-06-controlled-layout-selection.md](iq-cf-06-controlled-layout-selection.md)（选定自研最小组合器 A）。
...
| **CF-B1** | 图位文字追溯 | prompt/generation 产物记录文字所用 `fact_keys` 或本图覆盖；卖点一图一理由检查器（可先非 live） | 规格图「600ml」← fact `capacity`；保温杯卖点只强调「轻量杯身」且有依据 | 成片「24h 保温」无 fact；卖点同时堆三句无关口号且无 key | `完成`（`graph/text_trace*` + prompt/image 产物挂载；证据见 [compete-facts-text-trace](tasks/archive/compete-facts-text-trace.md)；OCR/采用硬闸非本批） |
...
- 用户于 2026-09-08 批准未知额度到期返还与检查警示后明确采用。本组保留质量检查、评分和冻结评测观测；确认采用不改变历史 fail 分数和原始结果。
...
- 成片 OCR B0 + 采用路径默认消费已交付；原采用硬闸曾收窄为期望字出现（残余墨迹不硬失败，见 [ocr-adoption-scope](tasks/archive/image-quality-ocr-adoption-scope.md)）；开放词表/CJK/生成路径自动 Apply 仍另发；≠R3。
...
用户于 2026-09-08 确认使用现有不同类别的真实电商图作为质量参照，目标为生成结果在各自类别和图种要求上不弱于对照。当前研发优先完善体验、效果与技术细节，外部反馈与商业指标不作为前置。新实现由 [分类图片标注](tasks/archive/category-image-annotation.md) 交付，复用既有数据和模型接入。

标注须区分商品身份参考、质量对照图和待评结果；共同事实/缺陷要求与类别展示要求分层。模型产出分维度评分、优点、缺点、具体依据、严重程度、不确定项和待验证建议；缺图、无法比较与模型失败保持可见，关键事实错误不得被均分掩盖。按类别/图种统计，保留样本身份与完整运行来源。模型评价和人工抽查分别记录，建议须经后续独立对照验证才能支持改进结论。

以下 42/32 图位等历史实验保留冻结公式、样本、分数与完成条件。新标注报告单独声明合同版本，不能把开发反馈当历史闸门通过，也不修改 Agent 人工标签合同。

2026-09-08 分类标注工具完成独立验收：8 类参考图和两类候选共 16/16 项有效多模态标注，候选比较 4 stronger / 4 mixed；分数、优缺点、依据、不确定项与建议均保留。参考图自身存在模型提示的型号不一致与身份材料不足，须抽查核实。初轮角色混淆失败和修正后原输入复验均留存，详情见上述归档任务。该结果不代表完整 195 商品池标注完成，也不更改历史生成质量门的结论。运行入口由 [Go 使用说明](../../go/README.md#分类图片标注) 维护。

同日 [三次重复判读](tasks/archive/category-annotation-repeatability.md) 共48/48项有效，但候选比较结论仅2/8图位完全一致，家电detail出现stronger/mixed/weaker三种结论；身份判断15/16一致。单次胜负不能采信为不弱于对照的证明。视觉抽查暴露型号/系列字符串误作矛盾、不同detail展示目的混比、类目检查点被误当每图必需项；下一步校准评委适用性后再指导生成链改动，不按波动排名盲调生产提示词。
...
生成内容策略候选见[归档](tasks/archive/image-quality-content-candidate.md)。空气炸锅与家纺[成对试验](tasks/archive/image-quality-content-pilot.md)已跑完（16/16 槽有分）；闸门未整体过线，工作台均分有升有降，不能写成已证明全面改善。
