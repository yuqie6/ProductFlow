...
当前 [image-eval-pool](tasks/image-eval-pool.md) 仍由原认领者执行，ID、池准入、评分阈值、凭据和浏览器占用不变。组织迁移不签收采集增量，不启动新 live。未提交的原图片账本记录原样迁入本文件，仍待原任务验收。
...
当前只有采证任务在执行，第三步不构成新实现授权。共用 provider/DB/浏览器资源仍需排他预约。工作流操作正确性与执行可靠性的固定合同继续适用，不复制 Graph 或图片执行器。
