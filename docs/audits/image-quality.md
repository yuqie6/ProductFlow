...
| IQ-CF-07 | **品牌/视觉继承优先级**：本商品显式覆盖 > 选定视觉方案版本 > 品牌版本 > 产品默认。事实与身份参考来自目标商品，**不参与**风格继承链。实例保存所选版本 id；品牌/方案更新先列受影响商品，须显式采用新版本，不得静默改在做任务与旧交付。配方继续清除来源商品身份与 fact/visual 版本绑定。 | 继承解析单测 + 配方 extract/apply 回归 + 第二商品无旧身份样例 | `完成`（`visualsystem.ResolveInheritance` 风格链只保留 style/colors；`product_visual_selections` 钉版本；Append 不静默改选择；Impact+显式采用；配方清身份与第二商品夹具；Brand 占位已由 `brand_table_not_ready` 改为 `brand_not_selected`/`brand_exists_no_style_merge`（商家平台 Brand B0）；证据见 [compete-facts-brand-inherit](tasks/archive/compete-facts-brand-inherit.md)；体验组 UX [brand-visual-reuse](tasks/archive/brand-visual-reuse.md)） |
