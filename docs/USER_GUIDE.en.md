...
`/products` opens the product directory with search, sorting and pagination. Switch to Merchant work overview for existing statistics, work records and source, state and date filters. Returning to the directory preserves product search and sorting; reload preserves the selected view. Each row prioritizes the product, category, price and last update. Its Actions button opens the cover filename and timestamps, with links to the workbench and the existing confirmed, permission-controlled deletion flow.
