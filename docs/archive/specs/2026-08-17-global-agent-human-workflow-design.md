...
本文负责 Global Agent、Session、Task、人工接管和 WorkflowRun 协作语义。Agent runtime 选型、Pi adapter、Skill、动态 Context、Tool 合同和 runtime 迁移验收由 `docs/adr/0007-pi-agent-runtime-boundary.md` 与 `docs/specs/pi-agent-runtime-integration.md` 负责。
...
- 将商品、工作流、图库和 Draft 能力按 Skill、动态 Context、ProductFlow Tool 三层暴露给 Agent；具体 runtime、目录、版本和验收规则见 `docs/specs/pi-agent-runtime-integration.md`。
- 有副作用的能力仍由 Tool 和后端绑定权限、scope、revision、confirmation policy、idempotency 和验证方式；Skill 只提供流程指导。
