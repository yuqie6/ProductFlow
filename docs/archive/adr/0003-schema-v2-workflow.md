...
已退出默认阅读路径。当时选择 schema-v2 的理由保留在此。在线图权威见 [`0008`](../../adr/0008-free-canvas-agent-graph-authority.md)；当前节点目录与写入入口见 `docs/ARCHITECTURE.md`。仍有效的 GenerationSpec / DeliverySpec / 一层分组 / 一图一节点约束写在 `CONTEXT.md`。

## 状态

Accepted。在线图权威已被 [`0008`](../../adr/0008-free-canvas-agent-graph-authority.md) 取代。下文「决策」记录当时选择 schema-v2 的理由，不再描述当前实现。
...
[`0008`](../../adr/0008-free-canvas-agent-graph-authority.md) 是 live graph、typed edge、Graph Command 和 GraphProposal 的决策。当前形状见 `ARCHITECTURE.md`。配方应用到 live graph 走同一套 Graph Command，不再先写成待确认 Draft。
