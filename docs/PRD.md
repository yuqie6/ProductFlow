...
7. 需要传统文案/海报链路时，仍可使用 `JobRun` 任务生成文案和模板/AI 海报。
...
3. 连接节点形成 DAG。
4. 启动后台工作流运行。
5. 在数据库中持久化运行、节点状态和失败原因。
6. 运行中前端轮询轻量 workflow status；结束后刷新完整工作流、商品详情和历史产物。
...
5. 在前端查看任务状态、工作流节点状态、生成队列概览和历史记录。
6. 在运行中使用轻量 status 接口刷新任务/工作流状态，避免高频拉取完整大对象。
7. 收藏连续生图结果到画廊，并通过受控下载接口取回原图。
8. 通过 `/settings` 保存业务配置覆盖，并避免在 API 响应中回显 secret 值。
9. 将上传/生成文件保存在本地 storage 目录，并通过受控下载接口读取。
