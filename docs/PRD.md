...
2. 走 Agent：系统创建商品、空的 WorkflowDraft、AgentSession 和 AgentConversation，并打开工作台对话。用户在对话框上传 1 至 6 张参考图，并说明图片类型；Agent 将其写入不可变 intake。创建页上的类型和参考图可作为进入对话前的可选快捷填写。
3. 跳过 Agent：在创建页选择图片类型、上传 1 至 6 张参考图、填写商品说明和出图设定，直接生成可运行画布。
4. Agent 检查已知信息并询问价格、风格、文字语种、文案要求、视觉体系等缺口。
5. Agent 输出商品事实、视觉体系、图片计划、每张图的提示词、参考绑定和生成规格。
6. 用户检查并确认草案。
7. 系统把确认后的草案写入 schema-v3 工作流图；同一段 Agent 对话继续留在工作台右侧。
...
- 走 Agent 时，商品名称即可进入对话；1 至 6 张通过媒体校验的参考图和图片类型在对话中提交（创建页也可作为可选快捷填写）。直接创建仍须在表单上提交参考图和图片类型。商品识别信息是否充分由 Agent 和用户审阅，后端不声称能自动证明图片真实性。
