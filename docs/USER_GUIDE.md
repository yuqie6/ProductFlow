...
顶部导航还提供 **画廊**。画廊不是另一个生成入口，而是用来收藏连续生图里已经满意的生成结果，方便之后集中浏览和下载。
...
满意后可以下载图片；如果还想连续微调，点顶部导航里的 **连续生图**。如果这张图来自连续生图，也可以保存到 **画廊**，后面集中浏览。
...
手机小屏上，连续生图页面会改成单栏：顶部操作、生成状态、结果预览、生成设置、参考图、画面描述和参数依次向下排列。生成多张候选时，结果区可以左右滑动查看。

### 8. 收藏到画廊

连续生图结果里可以把满意的生成图保存到 **画廊**。画廊会保留图片来源、关联商品、提示词、尺寸和模型信息，并提供下载入口。
...
- 连续生图进度闲置恢复阈值，默认 90 分钟；系统会按最近生成进度 heartbeat 判断运行中任务是否闲置。
- 上传文件大小限制。
...
文案、海报、工作流和连续生图都是后台任务。页面运行中会刷新状态，但不会反复下载完整历史数据：

- 连续生图运行中会更新队列位置、已完成候选数、最近进度时间、供应商状态、失败/成功状态和失败原因。
