...
- **底部生成面板**：包含生成设置和高级标签页。生成设置管理写回目标商品、会话参考图、画面描述、尺寸和候选数量；高级区域管理已启用的图片工具参数。底部提交按钮按当前候选数量发起生成。

### 8. 收藏到画廊

文/图生图结果里可以把满意的生成图保存到 **画廊**。画廊会保留图片来源、提示词、尺寸和模型信息，并提供下载入口。
...
进入顶部导航 **配置**，找到 **图片请求模板** 分组，可以调整长期默认的图片 provider 请求模板：
...
- 单次想要什么效果：写在提示词节点、生图节点或文/图生图输入里。
- 长期希望图片请求都遵循某种格式：调整配置页里的图片请求模板。
- 不确定怎么改：复制默认值，少量调整后保存测试。

恢复默认会删除数据库里的自定义值，回到系统默认图片请求模板。
