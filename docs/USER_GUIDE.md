...
1. **LaunchKit 快速流程**：面向越南 Shopee / TikTok Shop seller，生成可手动复制/下载的上架包。
2. **高级工作台教程与参考**：少术语，照着点、照着填，使用原商品画布生成文案和图片；完成一次流程后，可以继续了解工作台卡片、提示词配置、模型设置和常见问题。

产品内顶部导航提供 **帮助** 页面，适合在使用中快速查看流程、模板、支持边界和常见问题。这份 Markdown 文档保留为仓库内文字参考，内容应与产品内帮助页保持一致。

默认登录后会进入 **LaunchKit**：它不是自动上架工具，也不连接 Shopee/TikTok API；它帮你先把商品事实整理成 readiness 分数、Shopee/TikTok 文案块、图片证明计划、发布 checklist 和 Markdown 导出。原 **商品/工作台** 仍作为高级模式保留：中间是可缩放、可拖动的节点画布，桌面端右侧是一个精简侧栏，侧栏用小导航在 **详情 / 日志 / 图库 / 模板** 之间切换；移动端保留画布为主界面，用底部工具栏切换运行、单节点、模板、详情、日志和图库。

顶部导航还提供 **画廊**，用于收藏文/图生图里已经满意的生成结果，方便之后集中浏览和下载。

---

## LaunchKit 快速流程：准备一份手动上架包

目标：输入商品事实和卖家备注，生成可复制到 Shopee / TikTok Shop 的标题、描述、hashtag、图片拍摄/证明计划和发布前 checklist。

1. 登录后进入 **LaunchKit**，或点击顶部导航的 **LaunchKit**。
2. 点 **Tạo LaunchKit** / 新建入口，填写商品名、类目 playbook、目标平台、参考信息、参考 URL 和卖家备注。
3. 点 **Tạo nội dung**。生成完成后先看 **Điểm sẵn sàng**（readiness score）和警告，不要只看文案。
4. 在 **Xuất thủ công** 区域复制标题、描述、hashtag 或整块内容。需要离线交接时点 **Tải Markdown** 下载 Markdown。
5. 发布到平台后，在 **Feedback** 里标记是否使用、是否编辑、是否愿意复用/付费，并写下你改了什么。

LaunchKit v1 的边界：不自动登录平台、不自动上架、不调用 Shopee/TikTok API。它的核心价值是让 seller 在发布前更快检查事实、证明图片和可复制文案。
