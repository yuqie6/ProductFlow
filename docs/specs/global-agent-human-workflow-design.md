...
| `AgentTask` | 一个明确的业务目标，可跨页面、跨 Turn、后台运行 | 已实现独立记录、独立 harness run、列表/创建/改名/取消、工作台打开、全局素材整理/工作流关联 Draft 投影和 Agent 请求 WorkflowRun 的状态同步；暂停/恢复、Task 摘要、调度额度仍未实现 |
