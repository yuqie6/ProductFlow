...
| Agent 素材重命名的 mutation ledger 与对账 | `backend/src/productflow_backend/application/agent/tool_ledger.py`、`backend/src/productflow_backend/infrastructure/db/models.py`、`backend/tests/test_workflow_agent_service.py`、`backend/tests/test_live_agent_effects.py` | 真实 FastAPI + PostgreSQL 已验证响应体丢失后由 `AgentToolMutation` 账本裁决为 `applied`，重复请求复用同一结果且同一 conversation/key 只保留一条账本；没有匹配账本时仍保留冲突边界 |
