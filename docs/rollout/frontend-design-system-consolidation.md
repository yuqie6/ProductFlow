...
### 首切（TopNav）已回滚（2026-08-16）

- 现象：暗色导航头出现多余边框、字体变亮、底色过深。
- 根因：`index.css` 的 `:root.dark .*` 覆盖层是非分层 CSS，优先级高于 Tailwind 分层的 `dark:*` 工具类；首切按组件显式 `dark:*` 值做映射，而实际生效值是覆盖层值。
- 处置：`TopNav.tsx`、`index.css` 恢复到父提交 `7f37f64`，allowlist 恢复 59 项；导航头像素对比 `differingPixels=0, diffRatio=0` 确认恢复。
- 方法修订：见 spec「阶段 1 迁移方法修订」，后续切片必须按生效值规则推导 + A/B 基线快照验证，带疑禁止提交。
