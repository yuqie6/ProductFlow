...
2026-09-09 用户授权比较全部后台任务统一由 PG 驱动，或把更多通用调度交给 Redis/asynq。范围是 Graph 运行、图片会话生成、局部编辑、交付转码和 Agent Turn 同步。Kafka/RabbitMQ 不进入本次选型。当前生产实现仍是 PG outbox + Redis/asynq；本节是未完成的替代方案与验收合同。
...
候选固定 River 0.47.0。命令在既有 GORM 事务中解出同一个 sql.Tx，经 riverdatabasesql.InsertTx 创建带类型及商家身份的 job；Worker 通过同一入口调用现有五类业务执行器。活跃任务唯一性须绑定 actor、merchant_id、业务 ID 与执行轮；已完成后允许合法新一轮，迟到旧 job 不得触发新轮。外层回滚、GORM savepoint 回滚与新客户端消费已取得隔离证据，见平台可靠性章程。本地探针升级独立依赖，产品依赖尚未改变。
...
不能计入免费收益的部分：River Pro 的跨进程并发限制、工作流等功能不作为依赖；现有商家公平策略仍需适配。开源核心许可证为 MPL-2.0。候选要求 Go 1.26，当前产品 go.mod 与 Docker build 基线为 1.23；生产选择该版本时必须一并验证工具链、pgx 依赖、CGO/WebP 构建和发行路径，不能仅因开发机能跑就视为兼容。默认 job timeout/rescue 参数不对应现有长生图合同，须明确配置和实测。
...
两种机制均已通过最小可行性探针；River 另通过真实子 Worker 被 SIGKILL 后的结果不明恢复探针，未据此验收生产等待上限。不存在“unknown 必须禁用 Redis 重试”这一排他理由。River 事务接入可行，也不等于已经获得更低的完整维护成本。

当前优先进入方案 A 的五类适配设计与有界故障验证，尚不切换共享服务；方案 B 保留为完整替代方案。主要判别项是：保留业务公平性和恢复后，方案 A 是否仍能净删除投递与通用队列职责，且无需新增自研通用 Worker 框架。只有该条件及下述验收满足，才裁定统一 PG；若必须新建复杂调度框架或付费功能、损害公平性或业务读取，则裁定方案 B，不能迁移后补解释。

### 16.5 切换验收与并行边界

最终实现一次统一五类任务入口；临时验证在独立环境进行，不向产品加入长期双队列配置。以下是冻结的最终验收，不是当前通过声明：
