...
下表是 2026-09-07 SaaS 设计交接时的增量核对（代码观察 `0dff28fa`），不是研发完成百分比或整树复验。代码路径相对仓库根目录；详细当前合同仍由对应活文档维护。
...
| 身份 | `go/internal/auth/`、`schema/identity.go`；User 密码会话、Membership 与撤销 | 身份骨架已有；公开邮箱验证码注册、SMTP、账户恢复和完整页面需要增量交付 |
| 数据范围 | 商家根表 `merchant_id`、子链所有权、B0–B10 自动化隔离 | R1 历史门通过；`activeMerchantId` 仍取首成员，真正多标签页选择与媒体/SSE 传递需要验收；实例 settings 保持 Operator 权限 |
...
| 角色 | 允许 | 不允许 |
|---|---|---|
| 商家 Owner | 既有成员管理/移除、商家资料、查看额度与账单、生产编辑、数据导出 | 读取别的商家；修改站点 provider 密钥或自行增加额度 |
...
角色枚举和基础鉴权已实现，完整角色管理与运营流程尚未交付。最后一个 Owner 不能自行移除而留下无主商家；Owner 转移和成员移除必须有并发测试。新增成员的直接已有用户关联另行设计，本路线不以短期令牌或手工拉新作为入口。站点运维者对自身部署具有基础设施控制权，应用隔离不承诺防御宿主机管理员。运营支持读取商家内容需要显式支持会话、目的与审计，不提供隐式全局商家身份。

首版账号入口改为部署者 bootstrap 后的公开邮箱验证码注册：SMTP 由 Operator 配置，验证邮箱后设置密码，创建普通 User、其自有 Merchant、Owner Membership 和试用额度。复用现有密码与会话库，保留登录/退出和会话撤销，补注册节流、CSRF、安全 cookie 与自助恢复边界。注册切片已经实现，并已验证真实发信、邮件收取、注册事务、旧验证码重放拒绝和密码登录；自助恢复仍未实现。推荐和返利本阶段不实现，未来另行决定。
...
| 门 | 必须取得的证据 | 当前状态 |
|---|---|---|
| R1 身份与隔离 | 两商家、多角色、成员撤销；HTTP、资源、队列、事件、Agent 与后台任务交叉访问拒绝 | **通过**（2026-09-07；夹具双商 + B0–B10/MP-A/MP-B 自动化交叉拒绝；证据见 [merchant-r1-close-ruling](audits/tasks/archive/merchant-r1-close-ruling.md)；冻结基线残余非宣称见该文：`CreateMerchant` 仍 409 直至运营邀请（该邀请口径仅作历史快照，邀请流已退役）；≠MP-C/MP-D；≠开放互不信任第二商产品上线） |
...
在 R1 已过的候选上，完成第 5 节核心路径、交付选择、准确文字/布局基础、商家内视觉方案复用和 R3/R4 相应质量证据。公开注册在 Operator bootstrap 后显示，验证码交换和提交需在 SMTP 配置完成后按邮箱验证码创建普通账号、自有商家、Owner 和试用额度；对声明支持的图片类型、模型、设备和参考条件给出可核实限制。注册切片的真实浏览器、SMTP/IMAP 和 Web 回归证据已取得；外部发布仍需单独授权。
...
本节是公开注册试用版的目标设计，整体 SaaS 结果尚未实现；其中注册切片已经实现并有真实浏览器、SMTP/IMAP 和 Web 回归证据。原邀请试用设计已退役，相关任务与验收只作历史记录，不再作为入口或前置条件。设计者负责用户流程、关键取舍、共享不变量和最终验收；下述任务包供独立交接和固定采证，不要求每个内部实现步骤再建单。角色字段存在、接口返回 200、单个任务归档都不能代替本节的完整结果。

### 15.1 用户进入产品后的完整流程

| 使用者 | 入口与动作 | 完成结果及失败恢复 |
|---|---|---|
| 新注册操作者 | 在 Operator bootstrap 后提交邮箱，接收验证码并设置密码 | 验证成功后创建普通 User、自己的 Merchant、Owner Membership 和试用额度；重发或错误验证不会绕过 challenge 规则 |
| 日常操作者 | 首页查看待处理和近期商品，创建/改稿/采用/导出 | 回到同一商家和商品，生成前知道额度口径，失败可定位原任务 |
| 多商家成员 | 顶栏选择商家，或打开带商家上下文的深链接 | 每标签页独立，切换不改变已受理任务归属，迟到响应不串入当前商家 |
| 商家 Owner | 商家设置管理资料、既有成员；用量页核对消费 | 角色变更、移除、恢复有授权及冲突结果，任何时刻至少一位有效 Owner；新增成员的直接已有用户关联另行设计 |
| 个人用户 | 账号页改显示名、偏好、密码，管理会话 | 个人设置跨设备恢复；失去所有商家权限仍可退出和管理本人账号 |
| 站点 Operator | 运营列表找商家，核对余额/异常，处理停用、恢复或支持 | 不需要被加入每个商家；运营元数据和受授权业务内容区分，操作留有证据 |

公开注册试用首版必须提供以上流程；SMTP 字段和验证码规则已实现并通过真实注册链路验证。SSO、支付/退款、企业审批、商家 BYOK 和公开商家展示站不在本阶段。用户提出的“商家主页”在此指登录后的商家工作首页，不扩为对外商城。密码恢复、修改邮箱和团队新增成员的直接关联仍未接线，不能展示虚假的邮件成功提示或把人工邀请作为替代入口。推荐和返利本阶段不实现，未来另行决定。

### 15.2 页面与范围

下表为目标路由；已实现的 `/login` 注册模式进入当前能力，其余未接线页面不加入当前能力文档或展示空按钮。已有 `/products`、商品工作台、`/image-chat`、`/media-library`、`/settings` 继续复用。

| 目标页面 | 权限/范围 | 首屏内容与主要操作 |
|---|---|---|
| `/home` | 已选择商家的成员 | 待处理结果、近期商品、进行中的工作、余额摘要；创建商品与继续工作 |
| `/workspaces` | 已认证 User | 自己可访问的商家及状态；选择商家、进入个人账户；Operator 可进入运营 |
| `/account` | 本人，不要求 Membership | 资料、界面偏好、密码、本人会话；邮箱只读 |
| `/recover` | 未登录的账号本人 | 未来使用与注册用途分离的 SMTP 短期一次性凭据恢复密码；成功后撤销旧会话；当前未实现 |
| `/login` 注册模式 | 未登录公开入口；初始化完成后显示，SMTP 未就绪时发送验证码和提交注册禁用 | 请求邮箱验证码、设置密码和商家名；`display_name` 未提供时使用邮箱前缀；创建结果沿现有 cookie 登录 |
| `/merchant/settings` | 当前商家；Owner 写，成员按字段只读 | 资料与既有成员两个 tab；新增成员关联另行设计 |
| `/merchant/usage` | 当前商家 Owner | 可用/预留/已结算/待核对，分页流水与操作详情；所有成员仍可看余额和生成前单价 |
| `/ops/merchants`、`/ops/merchants/:merchantId` | Operator | 筛选商家、运营资料、额度及异常裁定；不直接展现商品或对话 |
| `/ops` | Operator | 待处理异常、停用商家、队列和资源情况，链接可操作的详情 |
| `/settings` | Operator | 原实例 provider、模型、容量配置；入口归运营导航 |

商家导航以首页、商品、素材为主；连续生图保留入口，品牌与配方按现有可用页面嵌入，未独立产品化前不造空页面。个人菜单固定在顶栏尾部；运营入口只向 Operator 展示。运营范围采用同一应用的导航变体，首屏明确“站点运营”，不携带隐式当前商家。

页面布局与视觉状态由 [工作流体验](audits/canvas-test-system.md) 负责，SaaS 专项原型尚未交付，实施时按本节流程和现有 Web 规范设计；账号/配置/权限/运营合同见 [商家平台 SaaS 设计](audits/merchant-platform.md#saas-platform-design)；容量预算与采证合同见 [多商家容量基线](audits/tasks/saas-multimerchant-capacity-baseline.md)，由平台可靠性负责。

### 15.3 验收口径

| 完整结果 | 需要的证据 | 会阻止公开注册试用的缺口 |
|---|---|---|
| 账号与工作商家可靠 | 当前已过：真实浏览器邮箱验证码注册事务、SMTP/IMAP 收取、登录保留、challenge 重放/错误/重发、普通 User 自有 Merchant/Owner session；Web 730 tests、lint/build 与 35 浏览器用例已通过。完整验收仍需邮箱验证码注册及失败恢复、多角色、两标签页、密码恢复/会话撤销、API/媒体/SSE 正反测试 | 验证码可重放、普通账号取得 Operator 权限、伪造转发头绕限制、跨商旧内容或凭据泄漏 |
| 商家能自行管理 | 资料与偏好持久化、成员并发变更、冲突恢复、桌面/手机浏览器操作 | 最后 Owner 丢失、权限撤销后继续消费、设置改错范围 |
| 消费和运营可解释 | 真实 DB 账本、重放与 unknown 竞争、运营审计、支持访问期限 | 重复调账、无限待核对、运营无记录读取商家内容 |
| 首页和概览可用 | 真实有界投影、口径/时区/刷新时间、待办可点入原对象、空错状态 | 假统计、计数口径不明、全表扫描造成业务接口失去响应 |
| 目标负载可承受 | 固定候选多商混合负载、故障恢复、商家间等待与资源记录 | 越权/重复结算任一发生；预算超限且无明确收窄后的支持范围 |

本表补充 S2 的完整 SaaS 体验出口，不改变 R1–R6 已有历史证据和冻结质量阈值。S2 验收必须同时列出本表、商品成图交付和 R3/R4 对应质量证据；其中任何未交付项均标缺口。没有真实商户反馈不宣称经营验证。

### 15.4 开发交接入口

首批任务发布于同一看板：身份入口与独立容量基线有现成前置；工作商家、账户团队、配置、运营明细按真实依赖解阻。开发协调者可在前置交付和文件/资源冲突检查后自主认领、实现、审核、提交，无需逐步询问用户。任务状态只维护在 [公共看板](audits/tasks/README.md)，本节不复制状态表。

商家首页/运营概览、受审计支持访问和更完整的成员关联保留为后续完整结果：已在组章程写出目标合同，待其读取和授权前置交付后，维护者在同一目标内发布具体实施包。不能以本批任务全部完成宣布整个 SaaS 完成。真实商家开放、外部发布、付费模型批次与收费经营仍需已有明确授权。
