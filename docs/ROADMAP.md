...
2026-09-09 用户授权比较全部后台任务统一由 PG 驱动，或把更多通用调度交给 Redis/asynq。范围是 Graph 运行、图片会话生成、局部编辑、交付转码和 Agent Turn 同步。Kafka/RabbitMQ 不进入本次选型。代码与本地开发服务均已统一为 River/PostgreSQL。本节保留选型依据及尚需逐项满足的切换合同。
...
短暂 PG 不可用及真实 SIGKILL 已验证，不据此宣称长期断连、主从切换或生产 SLA。生产 ImageSession 仍使用 90 分钟 stale 阈值，约一分钟的崩溃回归使用显式加速配置。本地开发服务已于 2026-09-09 切换。用户明确快速开发阶段不兼容旧任务：停止旧进程后退役 362 条旧队列记录，结束两个遗留执行并释放六条失效执行权；保留账号、483 个商品、素材及既有 unknown 结果。浏览器经真实 API 提交交付转码，River 单次执行成功并下载验证。

### 16.5 切换验收与并行边界

最终实现一次统一五类任务入口；临时验证在独立环境进行，不向产品加入长期双队列配置。以下保留冻结验收合同；代码与隔离验证证据见上一节，本地开发切换亦已完成：
...
- 开发环境切换停止旧进程，退役旧队列和废弃执行，保留账号、商品、素材与原始历史证据；不构建旧任务兼容或自动续跑机制。新版本仍保留业务幂等、商家隔离和 unknown 不重放。此策略不定义未来生产升级方式。
