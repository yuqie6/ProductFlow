...
当前尚未正式商用发布，没有真实商户用户、付费留存或获客数据。开发库中的商品、内部试用和评测样本均不得计作商户经营数据。User/Merchant 身份、业务隔离和内部额度已有实现，产品仍限制创建唯一开发商家；完整账户、运营与试用流程未交付。源码和用户入口的核对见第 2 节，完整 SaaS 设计增量见第 15 节。
...
| 身份 | `go/internal/auth/`、`schema/identity.go`；User 密码会话、Membership、邀请与撤销 | 身份骨架已有；邀请身份绑定、跨实例限流、账户恢复和完整页面需要增量交付 |
...
角色枚举和基础鉴权已实现，完整角色管理与运营流程尚未交付。最后一个 Owner 不能自行移除而留下无主商家；Owner 转移、邀请过期/撤销和成员移除必须有并发测试。站点运维者对自身部署具有基础设施控制权，应用隔离不承诺防御宿主机管理员。运营支持读取商家内容需要显式支持会话、目的与审计，不提供隐式全局商家身份。
...
出口：可以在当前支持范围完成首套图、局部改稿、交付和第二商品复用；部署与恢复演练有效；第 15 节的试用账号、商家管理、消费、反馈与支持流程均可用。用户安排真实邀请之前可以用内部完整任务核工程可行性，不冒充商户验证。
...
| 自营站点地区、支付与售价 | 内部邀请额度账本已有，完整邀请经营流程未交付；自动支付尚未选型 | 公开收费前由用户经营决策 |
...
本节是 2026-09-07 授权的目标设计，尚未实现。设计者负责用户流程、关键取舍、共享不变量和最终验收；开发协调者依据真实调用链连续实现、测试、集成和提交。下述任务包供独立交接和固定采证，不要求每个内部实现步骤再建单。角色字段存在、接口返回 200、单个任务归档都不能代替本节的完整结果。

### 15.1 用户进入产品后的完整流程

| 使用者 | 入口与动作 | 完成结果及失败恢复 |
|---|---|---|
| 受邀操作者 | 收到邀请链接，设置密码或以已有账号认证，选择商家 | 进入明确商家；过期/撤销可联系邀请者重签，不隐式登录已有账号 |
| 日常操作者 | 首页查看待处理和近期商品，创建/改稿/采用/导出 | 回到同一商家和商品，生成前知道额度口径，失败可定位原任务 |
| 多商家成员 | 顶栏选择商家，或打开带商家上下文的深链接 | 每标签页独立，切换不改变已受理任务归属，迟到响应不串入当前商家 |
| 商家 Owner | 商家设置管理资料、成员；用量页核对消费 | 邀请、角色变更、移除、恢复有授权及冲突结果，任何时刻至少一位有效 Owner |
...
邀请试用首版必须提供以上流程；公开注册、SSO、自动邮件投递、支付/退款、企业审批、商家 BYOK 和公开商家展示站不在本阶段。用户提出的“商家主页”在此指登录后的商家工作首页，不扩为对外商城。恢复凭据和邀请链接由有权限人员人工通过可信渠道交付；页面必须诚实说明交付方式。

### 15.2 页面与范围

下表为目标路由，未接线前不加入当前能力文档或展示空按钮。已有 `/products`、商品工作台、`/image-chat`、`/media-library`、`/settings` 继续复用。
...
| `/invite`、`/recover` | 对应用途短期凭据及身份验证 | 接受邀请或设置新密码；无营销内容、无未接线邮件成功提示 |
| `/merchant/settings` | 当前商家；Owner 写，成员按字段只读 | 资料与团队两个 tab；成员邮箱/邀请管理仅 Owner |
...
| 完整结果 | 需要的证据 | 会阻止邀请试用的缺口 |
|---|---|---|
| 账号与工作商家可靠 | 新/已有账号接受邀请、多角色、两标签页、密码恢复/会话撤销，API/媒体/SSE 正反测试 | 邀请可冒用已有账号、伪造转发头绕限制、跨商旧内容或凭据泄漏 |
...
商家首页/运营概览、受审计支持访问和第二商家产品开通保留为后续完整结果：已在组章程写出目标合同，待其读取和授权前置交付后，维护者在同一目标内发布具体实施包。不能以本批任务全部完成宣布整个 SaaS 完成。真实商家邀请、外部发布、付费模型批次与收费经营仍需已有明确授权。
