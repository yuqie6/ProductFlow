...
校准日期：2026-09-08。决策依据：用户确认的产品目标、`8b006abc` 源码、当前开发站只读观察及本页列出的竞品公开资料。本文是未实现方向、产品取舍和发布顺序的唯一总纲；当前能力仍以 [PRD](PRD.md)、[CONTEXT](../CONTEXT.md) 和 [ARCHITECTURE](ARCHITECTURE.md) 为准。业务组章程维护责任和验收证据，[公共看板](audits/tasks/README.md) 维护具体任务状态。
...
| 身份 | `go/internal/auth/`、`schema/identity.go`；User 密码会话、Membership 与撤销 | 公开邮箱验证码注册与 SMTP 已交付；账户恢复、本人账户与管理员商品页面仍需增量交付 |
| 数据范围 | 商家根表 `merchant_id`、子链所有权、B0–B10 自动化隔离 | R1 历史门通过；`activeMerchantId` 仍取首成员，账号自有范围与管理员跨商访问需要增量验收；实例 settings 保持 Operator 权限 |
...
普通账号注册后自有一个商家空间，登录后直接进入自己的商品。User 保存登录身份，Merchant 保存商品、素材、任务和额度归属；数据库分表不要求用户选择组织。既有 Membership 是实现遗留，收敛时保持历史数据可解释，不据其枚举扩展团队产品。普通用户加入多个商家、成员邀请、Owner/Editor/Viewer 团队和工作区选择退出当前范围。
...
管理员跨商家管理是明确的目标，完整后台和对应授权入口尚未实现。通过管理员权限访问目标对象并复用业务用例，不建立临时支持会话作为日常管理的前置。修改、删除和调账应记录操作人、目标、时间与结果；生成等付费动作的权限与费用归属单独说明，不能由跨商读取权限推导出来。
...
在 R1 已过的候选上，固定声明支持的图种与操作，验证首套图、指定修改、选定下载、下一商品复用和对应 R3/R4 质量。批量商品与完整文字编辑器独立验收，不扩大本次单商品试用分母；历史冻结评测结果保持原口径。公开注册在 Operator bootstrap 后显示，验证码交换和提交需在 SMTP 配置完成后按邮箱验证码创建普通账号、自有商家、Owner 和试用额度；对声明支持的图片类型、模型、设备和参考条件给出可核实限制。注册切片的真实浏览器、SMTP/IMAP 和 Web 回归证据已取得；外部发布仍需单独授权。
