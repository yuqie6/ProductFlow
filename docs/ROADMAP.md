...
现行采用路径中的有限 OCR 和语义判据强制拦截正在复核，用户是否可确认后继续采用待本轮明确裁定；不把该旧实现推广成新的通用产品要求。资产完整性与服务端权限仍是必要约束。
...
当前代码的 unknown 72h 全额结算已列为待用户裁定的政策，不能由后续任务默认沿用。确认新的用户额度处置之前保持代码事实与目标分开，供应商未知成本持续记录，不据到期推断零成本或成功。
