...
- 将现有图库升级为全局图库：用户明确保留的图片进入全局图库并长期积累。
- 让全局图库图片同步/关联到每个工作流的子图库；同一图片可被多个工作流使用，关联不复制媒体 bytes。
- 把旧顶层收藏 Gallery 迁移、切换并退休；保留工作流子图库和历史引用。
- 完成全局图库搜索、目录统计、批量移动、标签、归档/恢复和选择反馈。
- 为 Agent 图库整理工具补齐有界读取、可确认 Draft、原子应用和可解释结果。

### 5. 全局 Agent 与人工工作流协作

- 保留工作流画布的直接编辑、整图运行、单节点运行、取消、重试和运行记录入口；Agent 接入现有 `WorkflowRun`，不建立第二套执行器。
- 设计并实现跨页面 `AgentSession`、独立 `AgentTask`、本轮 `AgentTurn` 和页面上下文快照；Session 切换不取消后台任务，Task 目标不随路由变化。
- 为 Session 和 Task 建立有界列表、切换、归档、暂停、恢复、取消和待确认 Draft 投影。
