...
当前尚未正式商用发布，没有真实商户用户、付费留存或获客数据。开发库中的商品、内部试用和评测样本均不得计作商户经营数据。现有开发基线由 bootstrap 创建唯一开发商家；公开邮箱注册、运营 SMTP 与其账号/商家事务已经实现，并通过真实浏览器与真实 SMTP/IMAP 邮件收取验证，本人资料、密码恢复和会话管理已交付；个人偏好和管理员商品管理已交付，商家仪表盘与完整试用流程仍待完善。源码和用户入口的核对见第 2 节，完整 SaaS 设计增量见第 15 节。
...
| 商家仪表盘 | 以当前商家的真实商品、制作任务、待处理失败和已采用交付汇总工作；时间范围和筛选有明确含义，可进入原商品/任务继续处理；空白、加载、失败、窄屏均可用，不填充虚构销售或收入数据 |
...
仪表盘的具体聚合口径需对照现有持久状态确定，沿真实业务读者实现；尚未实现时保持目标状态。普通商家概览与管理员跨商概览分别授权，共用可复用查询逻辑，不构造跨账号的全局商品。
