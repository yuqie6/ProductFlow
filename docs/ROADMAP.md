...
当前尚未正式商用发布，没有真实商户用户、付费留存或获客数据。开发库中的商品、内部试用和评测样本均不得计作商户经营数据。现有开发基线由 bootstrap 创建唯一开发商家；公开邮箱注册、运营 SMTP 与其账号/商家事务已经实现，并通过真实浏览器与真实 SMTP/IMAP 邮件收取验证，完整账户、运营与试用流程仍未交付。源码和用户入口的核对见第 2 节，完整 SaaS 设计增量见第 15 节。
...
| 身份 | `go/internal/auth/`、`schema/identity.go`；User 密码会话、直接商家归属与会话撤销 | 公开邮箱验证码注册与 SMTP 已交付；账户恢复、本人账户与管理员商品页面仍需增量交付 |
...
注册继续使用已验证的 SMTP 邮箱验证码流程。密码恢复和账户自助管理为必要增量；不依赖团队功能。管理员配置页无需旧的二次解锁；未来若确有敏感操作需再认证，必须说明具体风险并由用户裁定，不能作为全部运营任务的隐含前置。
