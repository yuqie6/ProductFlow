...
- Global Agent Dock 已提供 Session/Task 列表、搜索、新建、归档、打开工作区、取消任务、暂停/恢复、Task 摘要和全局素材整理 Draft 投影/确认；业务级统一调度器仍待实现。Agent service 已通过 `AGENT_MAX_CONCURRENT_TURNS` 为所有 Task/Conversation Turn 设置进程级模型调用上限，等待中的 Turn 保持 durable queued。
