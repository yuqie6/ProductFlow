...
视觉变量由 `web/src/index.css` 统一拥有。保留现有配色、Inter + 中文回退；页面使用共享语义颜色，不自建重复色阶。组件整理与视觉重设计分开，未经用户明确要求不改变配色和表面关系。

复用共享表面、按钮和输入控件；保持已有卡片分组与侧栏结构。删除重复代码和文案时保留所有操作、输入状态和必要反馈。
