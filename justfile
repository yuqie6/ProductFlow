...
# Bytes stay under STORAGE_ROOT/image-evals/. Gate: docs/audits/image-quality.md#image-quality
...
image-evals-prepare-inputs out n="8" seed="1":
    bash scripts/with_dev_env.sh bash -lc 'go run -C go ./cmd/productflow-image-evals prepare-inputs --out "$1" --n "$2" --seed "$3"' -- '{{out}}' '{{n}}' '{{seed}}'
...
# Opt-in authenticated gallery HTTP ZIP memory gate with real PNG verification.
go-test-zip-http-rss:
    PRODUCTFLOW_RUN_ZIP_HTTP_RSS=1 bash scripts/with_dev_env.sh bash -lc 'go test -C go ./internal/mediaarchive -run TestGalleryArchiveHTTPMemory -count=1 -p 1 -v -timeout 4m'
...
# Merchant image loop: create → facts/text edit → generate → adopt → export → Brand reuse; temporary mock bind+restore.
web-e2e-delivery-merchant-image-loop *args:
    bash scripts/with_dev_env.sh bash -lc 'PRODUCTFLOW_RUN_CANVAS_WORKFLOW=1 PRODUCTFLOW_MERCHANT_LOOP_EVIDENCE_DIR="${PRODUCTFLOW_MERCHANT_LOOP_EVIDENCE_DIR:-$PWD/storage-dev/audits/delivery-merchant-image-loop-gate}" pnpm --dir web exec playwright test e2e/delivery-merchant-image-loop.spec.ts --config playwright.config.ts {{args}}'
